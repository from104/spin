...
- **공유 링크** — 드릴과 세션을 링크로 넘깁니다. **종단 암호화**라 열쇠는 URL 조각(`#`)에만
  있고 서버로 가지 않습니다 — 서버는 열지 못하는 암호문만 보관합니다. 직접 호스팅할 때는
  이 기능만 백엔드(`server/share`)가 필요합니다.
- **개발 중** — 데스크톱 앱(윈도우 · 리눅스 · macOS, Tauri)과 모바일 앱(iOS · Android · Chrome 웹앱)은
  아직 개발 중입니다. 지금 완성된 형태는 브라우저에서 쓰는 웹앱 하나입니다.
- **한국어 · English · 日本語**, 그리고 **키보드만으로 전부 조작 가능**합니다.

### 호스팅에 대하여

[spin.atit.app](https://spin.atit.app) 은 개인이 사비로 기여해 운영하는 주소입니다. **언제든 예고 없이
호스팅이 중단될 수 있습니다.** 그래서 코드를 공개합니다 — 팀이나 협회에서 계속 쓰려면
**직접 호스팅하세요.** 앱 본체는 정적 파일이라 어느 웹 서버에나 올릴 수 있습니다. **공유 링크를
쓰려면 `server/share` 백엔드를 같은 도메인의 `/api/share` 에 함께 올려야 합니다**(Node 22.18+,
의존성 없음, 종단 암호화된 덩어리만 보관). 그 밖의 기능은 백엔드 없이 전부 됩니다. 절차는
[docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) 의 배포 절, API 는 [server/share/README.md](server/share/README.md) 에 있습니다. 데이터는 이용자 기기와 본인
구글 드라이브에만 있으므로 주소가 바뀌어도 백업 파일로 옮겨 갈 수 있습니다.
