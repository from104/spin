...
> **2026-08-30 — 코트 바닥이 초록 잔디 → 나무(`#a9713c`)가 됐다**(기현 지시: *"일반 농구코트의
> 나무 바닥색으로"*). 아래 표·각주에 남은 `#1f7a46` 숫자들은 **그때의 기록**이다 — 지우지
> 않는다(대비 감사의 논증이 그 값 위에서 이뤄졌다). 바뀐 값만 여기 적는다:
>
> | 대비 | 초록 `#1f7a46` | 나무 `#a9713c` |
> |---|---|---|
> | 흰 라인(불투명) / 코트 | 5.34 | **4.11** |
> | `OBJ_STROKE`(흰 α.92) / 코트 | 4.78 | **3.75** |
> | `OBJ_STROKE_DARK`(검정 α.92) / 코트 | 3.75 | **4.83** |
> | `ARROW_CASING`(불투명 검정) / 코트 | 3.93 | **5.09** |
> | 메모 칩 `#0f1a14` / 코트 | 3.31 | **4.29** |
>
> 전부 §7.1 비텍스트 하한 3:1 위다. **밝은 쪽이 나빠지고 어두운 쪽이 좋아진다** — 바닥이
> 밝아졌으니 당연하고, 그래서 진짜 체육관 단풍나무(#d9b382 급)로는 못 간다: 거기서는 흰
> 라인이 1.96:1 이라 라인·개체 테두리를 어두운 색으로 뒤집는 재설계가 필요하다(기현님께 두
> 길을 제시하고 이쪽을 고르셨다). `#a9713c` 는 **기준을 지키는 가장 밝은 축**의 나무 톤이다.
>
> 함께 옮긴 것: 규칙 존 붉은 면의 판정 기준. 나무 코트는 그 자체가 붉은 계열이라 *"코트보다
> 붉어진다"* 가 성립하지 않는다(붉기 82.5 → 81.0). 면의 기능은 원래 흰 파선 테두리가 나르므로
> (`ruleZoneFill.test.tsx` 머리말) 재는 것을 *"코트와 다르되 연하다"* + *"위반 면이 평소 면보다
> 뚜렷하게 붉다"* 로 바꿨다 — 초록에서도 참이던 성질이라 잣대가 느슨해진 것은 아니다.

```ts
export const COURT_BG = '#a9713c';           // 다크·라이트 공통. 2026-08-30 나무 바닥
                                             // (옛 초록 #1f7a46 / 폐기된 라이트 #2f9e5c)
export const OBJ_STROKE = 'rgba(255,255,255,.92)';   // 흰선/코트 3.75:1  ※ 정정 2026-08-13·08-30
export const ARROW_CASING = '#000000';               // 검정(불투명)/코트 5.09:1
