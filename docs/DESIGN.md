...
| vbW, vbH | 800, 500 | 500, 425 | 500, 425 |
| surface | 25,25,750,450 | 25,25,450,375 | **0,0,500,425** |
| ruleZones | {25,150,125,200}, {650,150,125,200} | {150,275,200,125} | 없음 |
| goalPosts | (25,175)(25,325)(775,175)(775,325) | (175,400)(325,400) | 없음 |
| cornerCuts | `M25,50 L50,25` `M750,25 L775,50` `M775,450 L750,475` `M50,475 L25,450` | `M25,375 L50,400` `M450,400 L475,375` | 없음 |
| spotMarks | (112.5,250)(687.5,250) | (250,312.5) | 없음 |
| grid | 6×5, 125×90, origin(25,25) | 5×3, 90×125, origin(25,25) | **20×17, 25×25, origin(0,0)** |
...
Δ  = clamp(wrapPi(θr − θ), −ω·dt, +ω·dt);   θ' = θ + Δ
Pr2 = Gt − rho·u(θ' + beta)
...
Wake Lock·전체화면은 세션 전체에 걸쳐 유지한다. `N`/`Shift+→` 다음 드릴, `Shift+←` 이전.

### 6.10 편집기 도구 8종 — `src/features/editor/`

| # | key | 라벨 | 단축키 | 동작 |
|---|---|---|---|---|
| 1 | `select` | 선택 | `V`/`1` | 클릭 = 선택, Shift/⌘+클릭 = 토글, 빈 코트 드래그 = 러버밴드, **개체 드래그 = 4존 물리 조작** |
| 2 | `route` | 이동 | `R`/`2` | 드래그로 **실선 시안** 화살표(`move`) |
| 3 | `pass` | 패스 | `P`/`3` | 드래그로 **점선 앰버** 화살표(`pass`) |
| 4 | `ball` | 공 | `B`/`4` | 클릭 지점에 공 추가. **최대 10개** |
| 5 | `cone` | 콘 | `C`/`5` | 클릭 지점에 콘 추가. 2색, 무제한 |
| 6 | `player` | 선수 | `A`/`6` | **미배치 선수를 코트에 배치** |
| 7 | `note` | 메모 | `T`/`7` | 클릭 지점에 텍스트 메모 |
| 8 | `erase` | 지우개 | `E`/`8` | 클릭한 개체 제거 |
...
| `Ctrl/⌘+S` | 저장(자동저장 플러시), `preventDefault` |
| `Ctrl/⌘+D` | 현재 스텝 복제 |
| `←` `→` (개체 비선택) | 이전/다음 스텝 |
| `Space` | 재생/일시정지 |
| `G` / `Z` | 격자 / 골 지역 가이드 토글 |
| `Ctrl/⌘ +` `-` `0` | 스테이지 줌 인/아웃/리셋 |
| `Shift+?` | 도움말 오버레이 (`role="dialog"`, 포커스 트랩, `Esc`) |

**WCAG 2.1.4 (Character Key Shortcuts, Level A) 준수 필수.**
설정 → 접근성에 `단축키: 단일 키 / 수식키 필요(Alt+문자) / 끔` 3택
(`a11y.singleKeyShortcuts`). 키 디스패처를 `useEditorKeyboard` 한 곳으로 모으고 설정값에 따라
매칭 조건을 바꾼다. 음성 인식 사용자가 발화하면 `e`(지우개)·`b`(공)·`z`(토글)가 연쇄 발화한다.
**파괴적 동작(`E`, `Delete`)은 기본값에서도 수식키를 요구한다.**
...
- [ ] `easeStandard(0)===0`, `(1)===1`, 단조증가, `(0.5) ≈ 0.5` ±0.02, **1000회 샘플 비트 동일**
...
- [ ] **G6 spin 피벗 통과 안정성 (major 회귀)** — s=0.55 lat=0, 포인터를 축 위 +11.25 → −11.25 로
      ...
      `ε=±0.4 → θ = ±17.0188°`, `ε=±2 → θ = ±42.9838°` (±0.05). **부호가 ε 부호와 일치**
      (절대 추종이면 ±178°)
...
| G6 spin 피벗 통과 (ε=±0.4 / ±2) | θ = **±17.0188° / ±42.9838°** |
