...
| 2 | `line` | 선 | `L` | 드래그로 화살표. **세 앵커 모두 "끌면 옮기고, 누르면 바꾼다"** — 양 끝은 화살촉(없음→좁은→넓은), 굽힘점은 색(하늘→노랑→빨강, 2026-08-17) |
...
1. **오른쪽 버튼 / 길게 누르기 → 개체 메뉴 → [빼기] 또는 [삭제]** — 개체 종류별
   ...
   **[복제] (2026-08-18 기현 지시** — *"보드의 작도 객체, 메모 객체에 오른쪽 버튼 메뉴에
   복제 기능을 넣자. 복제하여 오른쪽 아래 1m 위치에 놓는거다"***)** — **도형·메모만**이고,
   사본은 원본에서 **오른쪽 아래 1 m(+25 px/+25 px)**, viewBox 에서 클램프(가장자리 사본이
   판 밖으로 나가면 잡을 수 없고, validate 의 로드 클램프와 기준이 갈리면 저장-로드에서
   자리가 튄다). 끝나면 **선택이 사본으로** 옮겨간다 — 다음 조작(끌어 자리 잡기)이 향하는
   곳이 방금 만든 쪽이다(스텝 복제와 같은 이유).

   낼지 말지는 필드가 아니라 `ids` 에서 계산한다(`canDuplicate` — '빼기냐 삭제냐' 와 같은
   규율). 칩·공·콘이 빠지는 이유: 정원이 cast(트레이 상자)에 있어 "하나 더" 는 배치가 아니라
   **정의 추가**다. 화살표는 지시 밖. 섞인 무리에서는 통째로 안 낸다(무시와 같은 정직 규율).
   구현은 새 액션 없이 `SHAPE_SET`/`NOTE_SET` 업서트에 새 id 를 실어 보낸다(`eraseIds` 가
   종류별 REMOVE 를 낱개로 쏘는 것과 같은 결 — 되돌리기도 사본 하나에 한 칸). 상한은 놓기와
   같은 문이다: 도형 40 은 placement 와 같은 토스트, 메모 20 은 놓기에 검사가 없지만 validate
   가 로드에서 자르므로 여기서 만들면 다음 로드 때 조용히 사라질 개체가 된다 — 막는다.
