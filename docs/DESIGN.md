...
> **정정(구조 개편 C5)**: 세션은 더 이상 드릴 목록의 2번째 탭이 아니다 — 레일 4번째 항목
> `sessions`(`#/sessions`, `features/sessions/SessionsScreen.tsx`)다. 본문(SessionTab)·드로어
> (SessionDrawer)는 그대로 이식됐고 드로어 열림은 URL(`?open=<id>`)이 저장소다. 헤더의
> [새 세션]이 비어 있지 않은 목록의 생성 진입점이다(빈 상태 CTA 는 본문에 유지). 드로어는
> C6(세션 전용 편집 화면)에서 은퇴 예정. 아래 원문은 탭 시절의 기록이다.
