...
초판은 판정 있는 조항에만 보드를 붙이고 나머지는 글자만 뒀는데, 그 조항들이 화면에서
휑했다(기현님 지적). 문제는 분량이 아니라 **코트 위의 사건이 아닌 조항이 상당수**라는 것이다
— 공의 규격, 장비, 경기 시간, 징계 절차에 억지로 판을 붙이면 빈 코트만 뜬다. 그래서
`RuleLaw.figureId` → `RuleFigure`(`Record<RuleFigureId, ComponentType>` 로 그림 누락을
컴파일에서 거른다) 로 **코트가 아닌 한 장짜리 정지 그림**을 붙인다. `sceneId` 와 배타가
아니라서 한 조항이 둘 다 가질 수 있고, 상세는 요약 → 도해 → 장면 순으로 쌓인다.

도해의 규율 둘: ① **수치를 손으로 적지 않는다** — 제2조 도해의 지름은 물리 엔진이 쓰는
...
스크롤이 죽으면 조항을 못 읽는다. 대신 도해는 **강제색에서 스스로 살아남게** 그린다: 구분을
전부 모양·라벨·치수선이 지고 색은 장식만 하므로, 색이 CanvasText 로 치환돼도 선 그림으로
읽힌다(§7 고대비 계약을 갈고리 없이 만족시키는 쪽).

**옆모습 파워체어 글리프는 조항을 넘나드는 공용 부품이다** (`figures/PowerchairGlyph.tsx`).
제2조(공기압) 도해가 먼저 그렸고, 제4조(선수 장비) 도해가 같은 체어를 재사용했다 — 실물
근거는 스트라이크포스(Power Soccer Shop 제작) 사용설명서다. `showAntiTip`·`showBelt`·
`showArmrest` 는 전부 기본 false 라 제2조 쪽 렌더는 그대로다. 제4조 도해는 이 글리프에
...
제2조 도해 안에 갇혀 있다가 제4조·제1조가 같은 모양을 필요로 하면서 밖으로 나왔다. 세
번째로 쓰는 자리가 생기면 뺀다는 규율이 여기서도 반복된다(`PowerchairGlyph.tsx` 가 먼저,
`Callout`·`Verdict` 가 그 다음).

**제1조(필드) 도해는 field-tour 장면과 겹치지 않는 것만 그린다.** 그 장면이 이미 골에어리어·
