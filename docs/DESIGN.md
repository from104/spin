...
| §3.5 | 렌더 레이어 순서 **고정, per-object z 없음** · `CURRENT_DRILL_SCHEMA = 8` | 스텝이 쥔 `zOrder` 목록을 `sceneOrder` 하나가 푼다(기본층 7종, 도형 포함) · 도장 **11**. 2026-09-06, `docs/PLAN-Z-ORDER.md` | `src/model/zOrder.ts`, `src/model/drill.ts` |
| §5.12 | 히트 우선순위 6단(**개체 종류가 서열**) · "가까운 쪽이 이긴다" | 4단 — 앵커 → 몸통을 `sceneOrder` **위→아래** → 존 핸들 → 휠체어 패드. 종류 서열 폐기. 2026-09-06 | `physics/hitTest.ts` |

> **§6.10d 이동 앵커(2026-09-13, 기현님 지시).** 고른 것을 통째로 옮기는 손잡이 하나가 개체
> 상자의 **화면 기준** 중앙 위에 뜬다(위가 모자라면 중앙 아래). 뜨는 자리는 넷 — 도형 하나·
> 메모 하나·**선 하나·자유선 하나**·여럿(⚠️ 2026-09-14 기현님 지시로 선·자유선이 더해졌다.
> 옛 근거 *"몸통 어디를 잡아도 끌린다"* 는 그 선이 **맨 위에 있을 때만** 참이었다). 휠체어·공·콘 **하나**에는 뜨지 않는다: 그 셋은 몸통을 잡으면 물리 드래그
> (견인·회전·충돌·리시)가 열리는데 앵커는 모델을 직접 미는 길이라 같은 손짓이 다른 이동이 된다.
>
> 앵커는 **히트테스트를 타지 않는다**(위 우선순위 표 밖이다). 밑에 무엇이 깔렸든 상관없다는
> 것이 겹침을 피하려고 만든 이유 자체이기 때문이다. 누름은 `PointerMeta.moveAnchor` 한 칸으로
> 컨트롤러에 전해지고, 옮기는 일은 §6.10b 덩어리 이동(`GROUP_NUDGE`)이 그대로 한다 —
> 새 이동 회로를 만들지 않았다. 명단은 `moveAnchorIds` 하나에서 오고(그리는 쪽·미는 쪽 공용),
> 자리는 `moveAnchorPlacement` 하나에서 온다. **기존 이동 방법은 하나도 줄지 않는다.**
>
> **§6.10e 비키기·가이드(2026-09-14, 기현님 실기 셋 중 둘).**
> ① *"원, 사각형은 회전 앵커와 겹친다."* — 회전 손잡이는 **월드 고정**(중심에서 h/2+16 월드 px),
> 앵커는 **화면 고정**(상자 위 21 CSS px)이라 화면 거리가 `|21 − 15·pxPerUnit|` 이다. 배율 ≈1.4
> 에서 정확히 0 이고 기본 배율이 그 근처였다 — 앵커가 위에 그려지므로 **회전 손잡이가 통째로 안
> 눌렸다**(실측: `elementFromPoint` 가 앵커를 돌려줬다). 상수로는 못 고친다(월드 상수 하나로 모든
> 배율의 화면 44px 을 살 수 없다). 그래서 앵커가 **가로로만** 비킨다 —
> `moveAnchorAvoidX`(순수)가 `x` **하나만** 돌려주는 것이 세로 규칙 불가침의 구조적 보증이다.
> 이격은 `2 × handleHitRadiusCssPx`(44) — 잡는 원 둘이 안 겹치는 하한이고, 33(=22+11)은
> «보이는 점만 눌린다» 라 표적 하한(--hit 44)을 스스로 깨는 값이라 쓰지 않는다.
> 비킨 앵커의 꼭지는 상자에서 **가장 가까운 점**을 가리킨다(안 비키면 정확히 수직 = 픽셀 동일).
> ② *"그룹화 했을 때 가이드 사각형이 필요하다."* — `SelectionGuide` 가 앵커와 **같은 상자**를
> 파선 + 모서리 갈고리로 그린다. 조건은 `moveAnchorGuide` 하나: **상자가 몸통과 다를 때만**
> (여럿 · 선 하나 · 자유선 하나). 도형 하나는 손잡이가, 메모 하나는 선택 링이 이미 상자를 말한다.
>
> 앞선 전제 두 가지를 먼저 고쳤다(`physics/bounds.ts`): 삼각형 상자는 `(x,y)` 를 중심으로 놓았는데
> 그 점은 **무게중심**이라 꼭짓점을 h/6 놓쳤고, 화살표 상자는 **화살촉**을 안 담았다. 겹침 판정에만
> 쓰일 때는 «메뉴가 한 칸 더 켜진다» 로 끝났지만, 이제 그 상자를 **그리므로** 개체를 자르고 지나간다.
...
> 한 자를 네 경로가 나눠 쓴다: `GoalPostMarks`(시연·인쇄) · `GoalPost`(편집기, 물리
> 바디라 로컬 좌표) · `buildStaticSvg`(PNG).
> ⚠️ 2026-09-06 정정 두 가지 — ① **썸네일은 골대를 안 그린다**(`COURT_LINE_WEIGHTS.thumb` 에
> `spotR` 이 없다. 20 px 그림에서 기둥 원은 얼룩이다). 위 목록에 썸네일이 있던 것은 사실이
> 아니었다. ② **골대의 층이 바뀌었다**: 그 전에는 `GoalPostMarks` 를 코트 라인 컴포넌트
> (`Full/HalfCourtLines`) 안에서 불러 격자·규칙 존·깃발보다 **아래**였는데, 편집 화면은
> `ObjectLayer` 가 골대를 그려 그 넷보다 **위**다. 그 차이 때문에 규칙 존의 흰 파선이
> 받침판 위를 가로질렀다(기현 지시: *"골대 밑판 위에 코트 라인이 보임"*). 이제 시연·인쇄·PNG
> 도 **규칙 표시 뒤·개체 앞**에서 그린다 — 네 경로의 층 순서는
> `src/render/courtFurniture.order.test.tsx` 가 편집 화면을 기준으로 잰다.
...
> ⚠️ 2026-09-06 — 위 사슬은 **개체**의 기본층이고, 판의 **부속**(사용자가 못 뒤집는 것)은 따로 한
> 사슬이다: `코트면(라인) → 격자선·번호 → 규칙 존(연한 면) → 진영 깃발 → 규칙 표시(링·위반) →
> 골대(받침판+기둥) → 개체…`. 정본은 **편집 화면**(CourtStage → ObjectLayer)이고, 시연·PNG·인쇄가
> 같은지는 `src/render/courtFurniture.order.test.tsx` 가 편집 화면의 순서를 읽어 와 잰다(여기
> 숫자를 다시 적지 않는다). 골대가 이 사슬의 그 자리로 온 경위는 §3.2 골대 각주 ⚠️ ②,
> 전체 대조 기록은 `docs/RENDER-PARITY-2026-09-06.md`.

> ── ⚠️ 2026-09-06: 위 *"고정, per-object z 없음"* 은 뒤집혔다 (`docs/PLAN-Z-ORDER.md`, 기현 지시
> *"드릴 편집에서 객체들의 표시 레이어조정 … 오른쪽 메뉴에서 표시순서 라는 하위메뉴"*) ──
>
> **전제가 죽은 범위만큼 뒤집었다.** 고정 순서의 근거는 `PLAN-2026-08.md` 「하지 않는 것」의
> *"1.3 겹친 휠체어 자가 분리로 대체된다"* — 판이 스스로 벌어지니 아래 것을 고를 일이 없다는
> 것이었다. 그 전제는 **휠체어끼리의 물리 충돌에만** 산다. 화살표·획·도형·메모·콘은 서로
> 밀어내지 않으므로 겹치면 아래 것이 영영 가려진다. 자가 분리는 그대로 두고 그 위에
> **사용자가 정하는 순서**를 얹는다.
>
> 새 계약(정본은 `src/model/zOrder.ts` 머리말):
> - `DrillStep.zOrder?: string[]` — **아래→위**, 7종 개체 id 를 한 목록에. **스텝별**이고, 첫
>   재배치 때 그 스텝의 전체 순서를 물질화한다. 개체마다 z 필드를 달지 않은 이유는
>   `locked`/`ignored`/`cut` 과 같다 — 저장 방식이 둘이 되면 정리 경로가 갈린다.
> - 유효 순서를 푸는 함수는 **`sceneOrder(step, cast): SceneRef[]` 하나뿐이다.** ① 목록이
>   없으면(옛 드릴 전부) 아래 기본층 순서 그대로 = **한 픽셀도 안 변한다**. ② 있으면 그 순서,
>   스텝에 없는 id·모르는 접두·중복은 무시. ③ 목록에 없는 개체(목록이 생긴 뒤 놓은 것)는
>   기본층 순서대로 **맨 위**에 붙는다(그리기 도구의 관례 — 방금 놓은 것이 위).
> - **기본층은 7종**이고 도형이 맨 아래로 합류한다: `도형 → 콘 → 획 → 화살표 → 휠체어 → 공 → 메모`.
>   도형은 `ShapeLayer` 를 떠나 `ObjectLayer` 의 정렬 목록 안에서 그려진다. 2026-08-14 지시
>   *"도형은 코트보다 높고 칩·화살표보다 낮게"* 는 **기본값으로 산다** — 사용자가 뒤집을 수 있게
>   되는 것만 바뀐다.
> - **판·시연·인쇄·PNG·썸네일 다섯 렌더 경로와 히트테스트가 전부 이 한 함수를 읽는다.** 경로마다
>   자기 정렬을 다시 짜지 마라 — 이 저장소에서 네 번 터진 "경로별 드리프트" 의 다섯째 사례가 된다.
> - 대상이 **아닌** 것: 코트면·격자·규칙존·골대·선택 강조·핸들(개체가 아니다). 이들의 상대
>   순서는 위 사슬 그대로다.
> - 스키마 **v10 → v11** 도장(아래 `CURRENT_DRILL_SCHEMA = 8` 은 v1 시점 값이다 — 도장의 정본은
>   `src/model/drill.ts` 다). 마이그레이션은 「⚠️ 적을 참말이 없는 상승」이고, 옛 앱은 `zOrder` 를
>   모른 채 고정 순서로 그려 **조용히 다른 그림**이 되므로 v11 문서를 `'too-new'` 로 거절한다.
...
× 장면 요소 17개(부속·개체 15 + 상태 표시 2, 2026-09-06)를 `render/renderPaths.ts` 의 `Record` 로 선언한다. 요소나 경로가 늘면 컴파일러가
...
> ── ⚠️ 2026-09-06: 위 6단 표는 **종류 서열**이었고, 그 서열은 폐기됐다
> (`docs/PLAN-Z-ORDER.md` 결정 9. 옛 표는 기록으로 남긴다) ──
>
> 개체 표시 순서(§3.5 ⚠️ 블록)를 사용자가 정하게 되면, 종류로 고정된 서열은
> *"앞으로 보냈는데 클릭은 뒤의 것이 잡힌다"* 를 만든다 — 그 기능의 뜻이 통째로 죽는다.
> 그래서 **몸통 판정은 종류를 안 보고 `sceneOrder` 를 위→아래로 훑어 처음 맞는 것을 준다.**
> 지금의 4단(정본은 `src/physics/hitTest.ts` 머리말):
>
> 1. **선택된 화살표·획의 앵커** (옛 3·3b). 개체가 아니라 조작점이고 선 **밖**에 그려지므로
>    몸통보다 앞이다 — 앵커 위에 공이 놓여 있어도 앵커가 이긴다.
> 2. **엄격 몸통** — 공·콘·메모 반경, 휠체어 OBB, 화살표·획 몸통 거리. `scene.order` **위에서부터**
>    첫 히트. 여기서 옛 1·2·6 이 한 단으로 접혔고 *"가까운 쪽이 이긴다"* 도 함께 폐기됐다.
> 3. **존 핸들** (옛 4) — 여전히 몸통 **뒤**. 이유는 아래 문단 그대로다.
> 4. **휠체어 hull + `grabPadPx`** (옛 5) — 맨 뒤. 순서 목록에 안 넣는 이유는 이것이 휠체어의
>    몸이 아니라 **잡기 여유**이기 때문이다(2 의 OBB 가 진짜 몸이다). 여유가 남의 몸을 이기면
>    "빈 곳을 짚었는데 옆 휠체어가 잡힌다" 가 된다.
>
> **관대(2차) 패스는 손대지 않았다** — 지우기 도구의 `forgivingRadius` 가드 포함(§9.3·9.4).
> §6.5 blocker(저배율 공 히트원이 휠체어를 덮음)도 그대로 안전하다: 공은 기본층에서 휠체어
> 위이고, 사용자가 공을 휠체어 밑으로 보냈다면 휠체어가 잡히는 것이 **맞는** 답이다.
...
> ── ⚠️ 2026-09-06: 뒷문장이 뒤집혔다 ── 렌더 순서(§3.5 ⚠️ 블록의 `sceneOrder`)와 클릭
> 우선순위는 이제 **같은 목록을 읽는다**. 독립적으로 정하면 사용자가 앞으로 보낸 개체를
> 클릭이 못 잡는다(§5.12 ⚠️ 블록). 앞문장의 기본층 사슬은 도형이 맨 아래로 합류해 7종이 됐다.
...
### 6.10 편집기 도구 11종 — `src/features/editor/`

> 2026-08-16 — 이동·패스가 **`line` 하나로** 합쳐졌고(뜻은 양 끝 화살촉이 나른다),
> 작도 도형 3종이 그 뒤에 섰다. 단축키는 같은 날 전면 개편됐다(§7.5f) — 숫자 키는 없다.
> 🔁 2026-09-03 — **9종 → 10종.** [지우기]가 되살아나 [메모] 옆 단독 버튼으로 섰다.
> 🔁 2026-09-03(2차) — **10종 → 11종.** [자유 그리기]가 [작도] 서랍의 `line` 옆에 섰다
> (기현 지시: *"드릴 편집 작도에 자유 그리기 추가"*). 서랍 **안**이라 첫 화면 표적은 안 는다.
...
| 3 | `freehand` | 자유 | `F` | **누른 자리부터 손이 지나간 궤적이 획**(`Stroke`)이 된다. 히트테스트를 안 돌린다 — 개체 위에서 시작해도 그 개체를 잡지 않고 거기서부터 긋는다. 표본은 2px 미만 이동을 버리고 손을 뗄 때 RDP(ε 1.5px)로 접는다(`simplifyPoints`); 총 길이가 탭 임계 이하거나 점이 2개 미만이면 **획을 안 만든다**. 스텝당 40획(`LIMITS.strokesPerStep`)에서 토스트로 막는다 — 넘겨 저장하면 다음 로드에서 validate 가 잘라 조용히 사라진다. 앵커는 **셋**(`line` 의 넷과 갈리는 자리다 — 획에는 굽힘점이 없다): 양 끝은 탭에 화살촉 순환(없음→좁은→넓은), 끝 접선 48px 바깥의 회전 앵커는 **끌면 회전**(축 = 잡는 순간의 `strokeCenter`, 래치)·**탭이면 색 순환**. 양 끝 앵커는 **끌어도 안 움직인다**(N점 획의 끝점만 옮기면 마지막 선분이 늘어나 모양이 깨진다). 몸통은 **탭이면 굵기 3단**(2.4/3.4/5.2)·**끌면 통째로 이동**, 그래서 몸통 탭은 재탭 해제를 타지 않는다(탭에 도는 값이 있는 개체는 탭이 해제로 안 흘러간다) |
| 4 | `shapeEllipse` | 원 | `O` | 작도 서랍 — `circle` 의 `c` 를 콘에 내주고 **oval** 로 본다 |
| 5 | `shapeTriangle` | 삼각 | `T` | 작도 서랍 |
| 6 | `shapeRect` | 사각 | `R` | 작도 서랍 |
| 7 | `ball` | 공 | `B` | 클릭 지점에 공 추가. **최대 10개** |
| 8 | `cone` | 콘 | `C` | 클릭 지점에 콘 추가. 2색, 색마다 8개 |
| 9 | `player` | 선수 | `P` | **미배치 선수를 코트에 배치** |
| 10 | `note` | 메모 | `N` | 클릭 지점에 텍스트 메모 |
| 11 | `eraser` | 지우기 | `X` | **클릭 한 번 = 그 개체 하나 삭제**, 도구는 유지(연속). 빈 곳 클릭·버튼 재클릭·`Esc` 면 `select` 로 복귀 |

**배치 도구는 하나 놓으면 선택 도구로 돌아간다**(§6.10a). 같은 도구를 한 번 더 누르면
고정되어 연속으로 놓인다 — `select`·`player`·`eraser` 는 고정 대상이 아니다.
`freehand` 는 **고정 대상이다**(`LOCKABLE_TOOLS`): 판에 손으로 덧그리는 일은 한 획으로 끝나는
법이 거의 없어(동선 하나에 곡선 서넛) 고정이 없으면 획마다 도구를 다시 골라야 한다.

**지우개 도구는 2026-08-16 에 제거됐다**(기현 지시).

> 🔁 **2026-09-03 — 되살아났다**(기현 지시: *"메모 옆에 (객체)지우기 버튼 추가. 클릭하고
> 보드로 커서가 가면 붉은 X 변화. 연속 삭제 가능. 빈 곳을 클릭하거나 다시 지우기 버튼을
> 누르거나 esc를 누르면 선택으로 복귀"*).
>
> **아래 제거 근거를 지우지 않는 이유는 그것이 그때의 참이기 때문이고, 지금 뒤집히는 이유는
> 그 두 전제가 각각 죽었기 때문이다:**
>
> 1. *"글자가 없다"* — 머릿글자 규칙을 이 도구에서만 놓았다. 화면이 붉은 `X` 로 말하므로
>    글자도 `X` 다(§7.5f). `X` 는 도구·개체 어느 층에서도 안 쓰이던 빈 자리라 아무것도
>    밀어내지 않는다.
> 2. *"삭제하는 문이 너무 많다"* — **그때 것과 다른 물건이다.** 옛 지우개는 선택과 겹치는
>    **드래그** 도구여서 "지금 무슨 모드인가" 가 손짓만으로는 안 보였고, 한 번 켜면 계속
>    켜져 있었다. 지금 것은 **클릭만** 받고(드래그·러버밴드 세션을 하나도 열지 않는다),
>    커서가 붉은 X 로 바뀌어 모드를 계속 말하고, 빈 곳을 한 번 찍으면 스스로 빠진다.
>    들어가고 나오는 값이 싸서 **상시로 켜 두는 모드가 아니다** — 문이 하나 는 것이 아니라,
>    "여러 개를 치울 때 잠깐 들어갔다 나오는 자세" 가 하나 생긴 것이다.
>
> **안전장치 셋** — 이 도구가 파괴 도구이므로 전부 명시적이다:
> - **관대한 히트 반경이 없다.** `forgivingRadius` 의 `tool === 'select'` 가드가 그대로다
>   (§9.4 F1 이 실검한 사고: 가드를 지우면 *지우개가 18 px 떨어진 공을 지운다*). 허용
>   목록이라 새 도구는 자동으로 엄격 패스만 탄다.
> - **핸들이 표적이 아니다.** 존 핸들은 차체 밖 45 px, 화살표 회전 앵커는 48 px 바깥에
>   앉는다 — 파괴 도구에서는 개체 **몸통**만 맞는다(`useEditorPointer` 의 eraser 분기가
>   `selectedChairId`/`selectedArrowId` 를 null 로 넣는다).
> - **고정(`toolLock`)되지 않는다.** `LOCKABLE_TOOLS` 가 허용 목록이라 자동으로 빠진다 —
>   연속 삭제는 이 도구의 성질이지 고정이 아니고, 파괴 모드가 잠긴 채 남으면 안 된다.
>   그래서 **[지우기]만 "같은 도구 한 번 더 = 고정" 관례를 비켜 간다**: 재클릭은 고정이
>   아니라 출구이고, 그 가로채기는 리듀서가 아니라 `ToolRail` 의 버튼에서 한다.
>
> **잠긴 개체는 지우지 않는다** — 고르기만 된다. 잠금의 뜻은 *"손으로 옮기는 것만 막는다"*
> 하나여야 하고(2026-08-14), 여기서 지워 버리면 *"지우기만 빼고 다 막는다"* 라는 셋째 뜻이
> 생긴다. 골라 주는 것이 안내다 — 개체 메뉴를 열어 잠금을 풀 수 있는 상태로 남는다.
>
> **삭제 경로는 새로 만들지 않았다.** 트레이 드롭·개체 메뉴·`Delete` 와 **같은**
> `eraseIds(ids, scope)` 로 들어간다(스코프는 트레이 드롭과 같은 `'onward'`) — 소리·토스트·
> 되돌리기가 입구마다 갈리지 않게 하는 유일한 방법이다. 도형만은 코트 히트테스트에 아예
> 없으므로(SVG 이벤트가 직접 받는 유일한 개체) `EditorStage.onShapeSelect` 에 같은 갈래가
> 하나 더 있다.
>
> **활성 표시가 붉다**(`#ff6b6b` — 개체 메뉴 [삭제]와 같은 값). 태블릿에는 커서가 없어서
> 버튼의 활성 표시가 "지금 누르면 사라진다" 를 말할 **유일한 자리**이기 때문이다.
...
> ⚠️ 위 `8×50` 은 **도구 8종이 한 줄씩 세로로 쌓이던 시절**의 검산이다. 2026-08-14 P3 로
> 기능 구역이 `row + wrap` 이 되면서 그 식은 화면과 무관해졌고, 지금 세로 합을 답하는 것은
> `trayMetrics.ts` 의 `trayFixedHeightPx` 다(하네스 전제를 `ToolRail.hit.test` 가 DOM 에서
> 대조한다). 2026-09-03 에 [지우기]가 늘어도 이 식이 안 움직이는 이유가 그것이다 — 버튼은
> 줄을 새로 만들지 않고 같은 줄에 흐른다. 표적 **수**의 변화(35 → 36)는
> `boardTargetBudget.test` 가 잰다.
...
> 🔁 **2026-09-03 — 위 표가 다시 유효해졌지만 `Alt` 열은 되살리지 않았다.** 되살아난
> [지우기]는 개체 하나를 **`eraseIds(ids, 'onward')`** 로 넘길 뿐이고, '이 스텝만' 을 고르는
> 수식키가 없다. 뺀 이유는 두 열이 화면에서 **구별되지 않기** 때문이다 — 둘 다 개체가
> 사라지는 그림이고, 어느 쪽이었는지는 다음 스텝으로 넘어가야 알 수 있다. 스코프를 정말
> 갈라야 하면 개체 메뉴가 그 자리다(거기서는 항목이 글자로 말한다). 마지막 두 행(선수는
> 미배치로, 코트 라인은 불가)은 그대로 참이다: 앞의 것은 `eraseIds` → `OBJECT_REMOVE` 가
> 원래 그렇게 동작하고(`returnsToTray`), 뒤의 것은 히트테스트에 그 대상이 없어서다.
...
🔁 2026-09-03 에 도구는 돌아왔지만 **드래그는 돌아오지 않았다** — 되살린 것은 클릭만 받는
모드이고, 여럿을 한 번에 치우는 길은 지금도 러버밴드 + `Delete` 하나다. 이 취소선은
**연속 삭제가 드래그로 돌아왔다는 뜻이 아니다**: 지금의 '연속' 은 지운 뒤 도구가 유지되어
다음 개체를 **또 클릭**할 수 있다는 뜻이다.
...
### 6.12 규칙 화면 — `src/features/rules/` (2026-08-21 신설, 2026-08-22 주제별 재설계, 2026-08-31 9카드 개편)

레일 4번째 항목 `rules`(`#/rules`, 세션과 설정 사이)로 v1 설계에는 없던 신설 화면이다.
FIPFA Laws of the Game(2025년판, `docs/RULES-FIPFA-2025.md` 가 정본)을 익히는 화면 —
**콘텐츠 정본 계획은 `docs/PLAN-RULES-9CARDS.md`**(2026-08-31, 카드 구성·산문 전문·장면
소스). 설계 히스토리 둘은 지우지 않고 남긴다: `docs/PLAN-RULES-REDESIGN.md`(2026-08-22,
카드 홈·블록 모델·재개 비교표의 원설계)와 `docs/PLAN-RULES-SCREEN.md`(2026-08-21, 화면
등록·라우팅·장면 인프라의 원설계).
...
갈아엎었다. 그때의 8주제는 `기본 규칙`·`경기 재개 한눈에`·`아웃 오브 플레이`·`골에어리어
반칙`·`2-on-1`·`그 외의 반칙`·`경합`·`공식 룰 북` 이었다. 조항 사전(18개조)은 마지막 주제
안의 **압축판 부록**(`LawIndexBlock`, 조항당 2~3줄)으로 강등됐다 — `ruleContent.ts` 는 이제
이 부록 하나만을 위한 데이터다.

**2026-08-31 9카드 개편 — 입구를 세우고 얇은 카드를 접었다.** 8주제는 구조로는 옳았지만
첫 카드가 `기본 규칙`(코트 28×15m)이라 이 종목이 무엇인지 한 줄도 없이 규격부터 시작했고,
`contested`(경합)는 산문 2문장 + 장면 1개로 카드 한 장 값이 안 됐다. 8주제를 고정한 그때의
기준은 *"18개조 사전을 학습 구조로 쪼갠다"*(쪼갤 대상은 정본 Law 1~18 뿐)였는데, 기현님
판정 *"최대한 간단·명료·쉽게"* 로 척도가 "처음 보는 사람이 첫 카드에서 뭘 아는가"로 바뀌면서
그 전제가 죽었다 — 종목 소개·경기 목적은 필요 없어서 빠진 게 아니라 **정본에 없어서** 못
들어갔던 것이다. 그래서 앞에 `intro`·`purpose` 를 세우고 `contested` 를 `restarts` 로 흡수해
**9카드**가 됐다. 현 `RuleTopicKey` 9개는 순서대로 `intro`(파워체어풋볼이란)·
`purpose`(게임의 목적)·`basics`(선수·코트·공·장비)·`out-of-play`(인·아웃과 득점)·
`restarts`(경기 재개)·`goal-area`(골에어리어 반칙)·`two-on-one`(2-on-1 반칙)·
`fouls`(그 외의 반칙)·`rulebook`(공식 룰 북)이다. 옛 딥링크 `/rules/contested` 는
`routes.ts` 의 단방향 흡수 분기가 `restarts` 로 받는다(그 분기가 없으면 북마크가 404 도 없이
조용히 카드 홈으로 떨어진다). 카드 1·2 의 일부 문장은 정본이 아니라 **연구자료**에서 나오므로
출처·자료 등급은 `docs/research/powerchair-football/README.md` 의 채택 문안 목록에 남는다.

**콘텐츠 모델은 `ruleTopics.ts` 의 `RuleTopic.blocks`(유니언) 하나다.** 한 주제는
`{prose|figure|scene|scene-slot|restart-table|card-list|law-index}` 블록의 배열이고,
`RuleTopicDoc` 이 그 순서 그대로 렌더한다 — 예전 "요약 → 도해 → 장면" 고정 순서가 "주제
저작자가 정하는 임의 순서"로 일반화됐다. `restart-table`/`card-list`/`law-index` 는 주제당
최대 1개(테스트가 강제)이고, 각각 `restartTable.ts`(재개 7종 비교 데이터)·
`MISCONDUCT_CARDS`(경고 7·퇴장 8)·`ruleContentFor()`(부록)를 그대로 읽는다.
`scene-slot`(2026-08-31 신설)은 **화면에 아무것도 그리지 않는 블록**이다 — 아직 장면이 없는
자리를 `blocks` 배열의 인덱스로 잡아 두되 "준비 중" 류 자리표시자는 배송하지 않는다. 필드가
`heading`/`body` 가 아니라 **`note`** 하나인 것도 그래서다: 다른 블록에서 그 두 이름은 화면에
뜨는 문구를 뜻하는데 여기서는 둘 다 버려지므로, 이름이 거짓말을 하면 다음 사람이 `heading` 을
채워 놓고 왜 안 뜨는지 찾게 된다. `note` 는 장면을 만들 다음 사람에게 남기는 메모다(시연
장면은 기현님이 드릴 편집기로 직접 만든다 — 2026-08-23 결정).
...
* **전역 도구 문자키**(`V L F O T R B C P N X` — `X` 는 2026-09-03 [지우기], `F` 는 같은 날 [자유 그리기]) — 예외가 없다. 설정 → 접근성의
