...
### 6.10 편집기 도구 10종 — `src/features/editor/`
...
| 10 | `eraser` | 지우기 | `X` | **클릭 한 번 = 그 개체 하나 삭제**, 도구는 유지(연속). 빈 곳 클릭·버튼 재클릭·`Esc` 면 `select` 로 복귀 |
...
* **전역 도구 문자키**(`V L O T R B C P N X` — `X` 는 2026-09-03 [지우기]) — 예외가 없다. 설정 → 접근성의
