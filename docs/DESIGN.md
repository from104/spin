...
1. **오른쪽 버튼 / 길게 누르기 → 개체 메뉴 → [빼기]** — 개체 종류별 액션(`ObjectMenu`).
   라벨이 '삭제' 가 아닌 이유는 **한 항목이 세 가지 뒷일을 덮기 때문**이다(2026-08-16 기현
   지시): 칩은 명단에 남아 트레이로 돌아가고, 공·콘은 어느 스텝에도 없으면 명단에서도
   사라지며(`pruneOrphanCast`), 화살표·메모·도형은 자기 액션으로 지워진다. '삭제' 는 칩에
   대해 **거짓**이었다 — 지우지 않는데 지운다고 적혀 있었다. 아이콘도 `✕`(없앤다)에서
   `←`(판 밖으로 물린다)로 바꿨다. 붉은 색은 남긴다: 근거가 비가역성이 아니라 **판에서
   사라진다는 사실 자체**로 옮겨졌을 뿐이다.
