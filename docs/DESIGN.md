...
지우개 도구가 없어졌다(§6.10). 여러 개를 한 번에 치우는 길은 러버밴드 선택 + `Ctrl/⌘+Delete` 다.
...
클릭 · `Enter` 토글)인데, 그렇게 고른 것으로 할 수 있는 일은 `Ctrl+Delete` **하나뿐**이었다.
...
- **도형이 키보드로 안 지워졌다.** `eraseIds` 에 `sh` 갈래가 없어 `Delete`·`Ctrl+Delete` 가
