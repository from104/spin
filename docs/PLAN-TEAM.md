...
| 17 | **화면 구조**: 목록(카드: 색 견본·이름·약칭·선수 수·PF 칩·스태프 수, 메뉴 열기/복제/내보내기/인쇄/삭제, 빈 상태 [새 팀]·[가져오기]) → 상세(헤더 인라인 이름·약칭·색 2종 ⟶ ⚠️ 2026-09-09: 헤더가 **반 폭 카드 둘**(«기본 정보» + «팀 색»)로 갈렸고, 팀 색 카드는 팔레트 행 + 킷 행 3개이며 슬롯마다 **세로 팝업**으로 색을 고른다(`KitPicker.tsx`), 절: 선수 / 스태프 / 라인업 / 정보). 선수 절은 **인라인 추가 행 + 행 클릭 편집 + 검색 + 정렬(등번호/이름/등급) + 비활성 보기 토글**. 전역 «현재 팀» 상태는 만들지 않는다 | 지시 «다루기 쉽고 인터랙티브». 조사 Q4·§2.5(SquadGod 드래그·TeamSnap 필드) |
...
- **팀 색**(2026-09-09): 팔레트 색 선택기를 **드래그하는 동안** 저장이 안 나가는가(색이 눈앞에서 튀지 않는가), 팔레트 색을 고치면 그 색을 쓰는 킷 셋이 함께 바뀌는가, 좁은 창에서 킷 3줄이 접히고도 표적 44px 을 지키는가, 색 팝업이 키보드로 도는가(↓/Enter 로 열기 · ↑↓ 이동 · Enter 선택 · Esc 로 닫고 초점 복귀), 화면 아래쪽 카드에서 팝업이 **위로 뒤집히는가**(jsdom 은 rect 가 0 이라 못 잰다), 넓은 창에서 «기본 정보»·«팀 색» 두 카드가 각각 절반 폭으로 서고 좁은 창에서 한 열로 접히는가, 팀시트 킷 표가 흑백 복사에서 읽히는가.
