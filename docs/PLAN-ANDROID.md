...
**A-13~A-15 는 배율 작업(2026-09-19, 정본 [`PLAN-UI-SCALE.md`](PLAN-UI-SCALE.md))이 더한 것이다.**
에뮬레이터 셋(7"·10.1"·iPad mini)으로 이미 쟀지만, 에뮬레이터는 **밀도를 숫자로 흉내낼 뿐 손가락이
없다** — 아래 셋은 그 차이에서만 드러난다.

- ☐ A-13 「UI 크기 = 자동」이 실기에서도 옳은 눈금을 고른다 — [보드]에서 풀코트 판이 상하좌우 꽉 차고,
  왼쪽 레일과 아래 트레이 **어느 쪽에도 스크롤이 안 생긴다**(결정 3 의 정의 그대로). 가로↔세로를 돌려도
  각 방향에서 다시 그렇게 된다. ⚠️ 한 눈금 낙관적으로 나오는 실패가 실제로 있었다(`clientHeight` 가
  safe-area 패딩을 포함해 2026-09-19 에 744 vs 676 으로 갈렸다) — 「거의 맞다」 는 통과가 아니다
- ☐ A-14 헤더 오른쪽 버튼(높이 **29px** = `--hit-slim`)이 손가락으로 눌린다 — 44 아래로 내린 것은
  기현님 수락 아래의 결정이다(2026-09-19). 「눌리긴 하는데 자꾸 빗나간다」 면 그것이 되돌릴 근거다.
  마우스·에뮬레이터 클릭으로는 **영원히 재현되지 않는다**
- ☐ A-15 레일이 넘칠 때 **스크롤로** 해결된다 — 가로에서 배율을 175~200% 로 올려 레일을 넘치게 한 뒤,
  끝 항목([설정]·버전 글씨)까지 닿는지 본다. 항목이 **쪼그라들어** 들어맞으면 실패다(`flexShrink: 0`
  회귀 — 2026-09-19 에 44px 목표가 20px 로 찌그러진 적이 있다)
