...
파워체어 풋볼 훈련 도구 SPIN 이 **무엇을 하는 앱이고, 왜 이런 모양인가**를 적습니다.
[README](../README.md) 가 "쓰는 법"이라면 이 문서는 "이렇게 만든 이유"입니다.

첫 독자는 **나중의 나와 AI 에이전트**입니다. 세션이 끊기고 몇 주 뒤에 돌아왔을 때,
혹은 이 저장소를 처음 여는 사람이 **코드를 읽기 전에** 알아야 할 것들을 한 파일에
모읍니다: 무엇을 만들고 있는가, 왜 이런 모양인가, 어디를 봐야 하는가.

세부는 전부 `docs/` 에 있고 이 문서는 그리로 가는 지도입니다. 다만 **지도만 있고 설명이
없으면 매번 8,500줄을 다시 읽어야 하므로**, 판단의 뼈대는 여기 옮겨 적습니다.
개발·검증·배포 절차는 [DEVELOPMENT.md](DEVELOPMENT.md) 에 따로 있습니다.
...
딥링크됩니다. 규칙 팩트 정본은 [docs/RULES-FIPFA-2025.md](RULES-FIPFA-2025.md), 콘텐츠
정본은 [docs/PLAN-RULES-9CARDS.md](PLAN-RULES-9CARDS.md)입니다(옛 설계는
[docs/PLAN-RULES-REDESIGN.md](PLAN-RULES-REDESIGN.md)·[docs/PLAN-RULES-SCREEN.md](PLAN-RULES-SCREEN.md)
에 히스토리로 남습니다). 카드 [파워체어풋볼이란]·[게임의 목적]의 일부 문장은 정본 밖
연구자료에서 나오므로, 그 출처·자료 등급은
[docs/research/powerchair-football/README.md](research/powerchair-football/README.md)
의 채택 문안 목록에 적혀 있습니다.
...
[docs/DESIGN.md §7](DESIGN.md) 이 **계약**으로 못 박고 있고, 테스트가 지킵니다:
...
- ~~온라인 공유·협업 (서버가 없는 것이 설계입니다 — 파일로 옮깁니다)~~
  → **2026-09-08 에 절반이 뒤집혔습니다.** 공유 **링크**는 있습니다(`server/share`, 종단 암호화 —
  서버는 열쇠 없는 암호문만 보관합니다). "서버가 없다" 는 원칙은 **앱 데이터**에 대해서는
  그대로입니다: 드릴·세션·명단은 여전히 브라우저에 있고 서버는 읽지 못합니다.
  실시간 **협업 편집**은 여전히 없습니다. 근거는 [PLAN-SHARE-LINK.md](PLAN-SHARE-LINK.md).
- 실시간 경기 기록·통계
