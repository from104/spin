...
//
// `localStorage` 에 두지 않는 이유: 그 저장소는 페이지 스크립트라면 무엇이든 읽는다. 파일로
// 내려두면 최소한 **웹 문맥에서는 못 읽는다** — 커맨드를 거쳐야만 닿는다.
//
// ⚠️ 2026-09-05 뒤집힘: 이전에는 여기서 "지금 앱에 외부 스크립트가 하나 있고(구글 GIS)"
//    라고 적었었다 — 틀렸다. 데스크톱 로그인은 처음부터 **외부 브라우저**로 여는 흐름이라
//    GIS(구글 identity 스크립트)를 앱 웹뷰에 실은 적이 없다(`src/sync/authDesktop.ts` 의
//    `openInBrowser`). 그 오독 위에 CSP 의 `script-src`/`frame-src` 에 구글 도메인을
//    허용하는 죽은 완화가 얹혔던 것도 같이 걷어냈다(`docs/AUDIT-DESKTOP-MERGE-2026-09-05.md`
//    §③ [치명 3]). 로컬 파일을 고집하는 이유 자체는 안 바뀐다 — 웹뷰 문맥에 실릴 어떤
//    스크립트(지금은 없지만 나중에 생길 수 있는)로부터도 갱신 토큰을 떼어 두는 것이 목적이다.
//
...
fn lock_down(p: &PathBuf) -> Result<(), String> {
    ...
    fs::set_permissions(p, fs::Permissions::from_mode(0o600))
        .map_err(|e| format!("파일 권한을 설정하지 못했습니다: {e}"))
}
#[cfg(not(unix))]
