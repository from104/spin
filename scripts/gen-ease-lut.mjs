...
// easeStandard = cubic-bezier(.4,0,.2,1) (§3.6/§6), i/128 로 i=0..128.
const ease = cubicBezier(0.4, 0, 0.2, 1);
const lut = Array.from({ length: 129 }, (_, i) => ease(i / 128));
const cells = lut.map((v) => v.toFixed(12));

if (process.argv.includes('--check')) {
  const src = readFileSync(new URL('../src/core/geom.ts', import.meta.url), 'utf8');
  const block = src.match(/EASE_STANDARD_LUT: readonly number\[\] = \[([\s\S]*?)\];/);
  if (!block) {
    console.error('geom.ts 에서 EASE_STANDARD_LUT 를 찾지 못했다');
    process.exit(1);
  }
  const have = block[1]
    .split(',')
  ...
  if (have.length !== cells.length || bad >= 0) {
    console.error(`불일치: 길이 ${have.length}/${cells.length}, 첫 어긋난 자리 ${bad}`);
    process.exit(1);
  }
  console.log(`일치 — ${cells.length}점`);
} else {
