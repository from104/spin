...
# spin.atit.app A → 54.180.213.12)는 이미 구성되어 있고 다시 만들 일은 없다.
#
# 사용법:
...
#
# 환경변수로 대상을 바꿀 수 있다:
#   SPIN_AWS_HOST(=54.180.213.12) · SPIN_AWS_USER(=bitnami) ·
#   SPIN_AWS_PEM(=~/.ssh/aws-mocil-test.pem) · SPIN_AWS_PATH(=/opt/bitnami/apache2/spin-htdocs)
set -euo pipefail

HOST="${SPIN_AWS_HOST:-54.180.213.12}"
USER="${SPIN_AWS_USER:-bitnami}"
PEM="${SPIN_AWS_PEM:-$HOME/.ssh/aws-mocil-test.pem}"
