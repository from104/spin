...
- 시드 드릴·초안 문서는 제목에 승인 상태를 적는다(`SEED-DRILLS-DRAFT.md` — "기현님 승인 대기").
  - ⚠️ 2026-09-06: 이 줄이 가리키던 초안은 아래 문단의 지시로 폐기됐다(문서 삭제). **승인 대기라는
    상태 자체가 사라졌다** — 시드는 이제 승인받을 별도 콘텐츠가 아니라 규칙 화면 장면의 사본이다.
    다른 초안 문서에 승인 상태를 적는 관행은 그대로 산다.
...
**첫 실행 시드의 정본은 규칙 화면 장면 목록 하나다** (2026-09-06 기현 지시: *"첫 실행시 기본
저장되어있는 드릴을 규칙에 있는 드릴로 교체 (앞으로 쭉 그 정책 유지"*). 심는 드릴은
`RULE_SCENE_IDS` 에서만 나온다 — 규칙에 장면을 넣으면 시드에도 들어가고, 빼면 시드에서도
빠진다. 시드 전용 드릴 목록도, 시드 전용 손코딩도 앞으로 없다(이 절의 연장이다: 시드도 코트 위
장면이므로 편집기로 만든 것만 심는다). 심는 **id 와 시각은 고정**한다 — 기기마다 새 id 를
발급하면 동기화 뒤 기기 수만큼 사본이 생기고, 심는 시각이 매번 새로우면 LWW 가 다른 기기에서
고친 것·지운 것을 시드로 덮어쓴다. 이미 쓰던 기기(`prefs.seeded` 도장이 찍힌)에는 **소급하지
않는다** — 재시드 장치를 만들지 않는다. 정본은 `docs/PLAN-SEED-FROM-RULES.md`.
...
- **`src/core/keymap.ts` 는 NUL 바이트(중복 제거 키 구분자)가 있어 `rg` 가 바이너리로 건너뛴다
  — 그 파일을 훑을 땐 `rg -a`.**(2026-09-08, `docs/PLAN-HELP-OVERHAUL.md` F9)
...
- **번들에 인라인되는 값이 CI 에서 비면 아무 표시도 안 난다.** 없는 시크릿을 GitHub 은 빈
