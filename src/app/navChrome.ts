...
export const HEADER_PAD_PX = {
  // ⚠️ 2026-09-18 기현님 지시(*"공통 헤더의 높이를 2/3로 줄이자"*)로 상하 8 → **2**. 좌우 24 는
  // 그대로다 — 줄이라는 것은 높이였다. 이로써 표준 헤더도 44 + 2·2 = **48** 이 되어 코트 화면의
  // 컴팩트 헤더와 같은 높이가 된다(AppHeader 의 minHeight 48 과 한 쌍이다 — 한쪽만 고치면
  // 여백이 남거나 표적이 잘린다).
  //
  // 지시는 2/3(41)이었으나 41 은 **낼 수 없다**: 안에 서는 것이 `--hit`(44) 짜리 표적이라
  // 여백을 0 으로 깎아도 44 가 바닥이다. 48 이 그 바닥에 가장 가까운 값이고(62 → 48, −23%),
  // 더 줄이려면 표적을 44 미만으로 내려야 하는데 그것은 이 파일이 지키는 선이다(아래 narrow
  // 주석과 같은 산술). 기현님께 이 산술을 보이고 48 통일을 택하셨다.
  wide: { x: 24, y: 2 },
