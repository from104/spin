...
export const CHROME_ROWS: readonly ChromeRow[] = [
  ...
  },
  {
    ...
    label: '앱 헤더',
    now: 62,
    // ⚠️ 2026-08-20 — **이 `wide` 는 더 이상 코트 화면(자유 전술판·드릴 편집)에 안 쓰인다.**
    // 아래 chromeRowPx 의 appHeader 분기가 `row.narrow` 만 읽는다 — 코트 화면의 헤더는
    // 이제 넓든 좁든 컴팩트(48)뿐이다. 이 값은 목록·세션·설정처럼 **코트가 없어 이 예산표
    // 자체를 안 쓰는 화면들**의 표준 헤더 값으로만 남는다(문서용 리터럴 — chromeRowPx 가
    // 이 필드를 읽지 않는다는 것은 chromeBudget.test.ts 가 못박는다).
    //
    // ⚠️ 2026-09-18 기현님 지시(*"공통 헤더의 높이를 2/3로 줄이자"*)로 62 → 48. 이어서 같은 날
    // 후속 지시(*"헤더의 오른쪽 각 버튼들 높이도 2/3로 줄여"*)로 오른쪽 묶음이 29(=--hit-slim)가
    // 되면서 바닥이 한 번 더 내려가 **33**(29 + 여백 2·2)이다. 좁은 창은 왼쪽 세그먼트(44)가
    // 지배하므로 narrow 48 은 그대로다 — 그래서 wide < narrow 라는 낯선 모양이 된다. `now`(62)는 건드리지 않는다
    // — 재편 **이전** 실측값이라 §5.3 '현재' 열과 CHROME_HEIGHT_NOW_PX(196)가 그 값으로
    // 계산된다. 즉 이 변경은 예산 **합계를 바꾸지 않는다**(chromeRowPx 가 narrow 만 읽는다).
    wide: 33,
    ...
    //
    // ⚠️ 2026-08-20 (기현님 지시) — **드릴 편집도 넓은 창에서 이 행을 도로 얻는다.** "편집·
    // 시연 화면이 비슷한 레이아웃이어야 ux가 좋아진다"는 지시로 두 화면이 같은 컴팩트 헤더를
    // 쓴다 — 아래 chromeRowPx 의 조건이 `state.narrow` 단독 판정에서 `state.board` 판정으로
    // 바뀐 이유다. 자유 전술판(board)만 넓은 창에서 0 이고, 그 밖(드릴 편집 포함)은 항상 48.
    narrow: 48,
    owner: '2026-08-20 — 드릴 편집·시연 공용 컴팩트 헤더(§A). 넓은 전술판만 예외로 0.',
  },
  {
  ...
  },
  {
    ...
    now: 0,
    // ⚠️ 2026-08-20 (기현님 지시, §D) — 45(토글 줄 --hit 44 + 경계선 1) → **61**(60px 재생
    // 버튼 + 경계선 1). 이 줄이 이제 노트뿐 아니라 공용 PlaybackControls(반복·이전·재생·
    // 다음·배속)도 진다 — `[NotePanel flex:1][PlaybackControls flex:none]` 한 줄이라 행
    // 높이는 둘 중 큰 쪽(재생 묶음의 60)이 정한다. 창 폭과 무관하다(재생 묶음은 좁혀도
    // 안 작아진다) — 그래서 wide·narrow 가 여전히 같은 값이다.
    wide: 61,
    narrow: 61,
    owner: '2026-08-20 — 노트 + 공용 재생 묶음(§D·E, 2026-08-17 접이식 노트의 후계)',
  },
  {
    id: 'courtPadY',
    axis: 'height',
 ...
 *
 *  ⚠️ 2026-08-20 (재생 묶음 공용화, §D) — **109 → 125.** 좁은 창에서는 appHeader 행이 이미
 *  narrow 값(48)이라 헤더 쪽은 그대로다 — 늘어난 16 은 전부 notePanel 45→61 몫이다. */
export const CHROME_HEIGHT_NARROW_PX = 125;
...
export function chromeRowPx(row: ChromeRow, state: ChromeState): number {
  ...
  // ★ 자유 전술판만 **넓은 창에서 헤더가 없다**(AppShell 의 showHeader).
  //   좁은 창에서는 어느 화면이든 남는다: 84px 레일이 빠진 자리를 헤더의 3칸 세그먼트가
  //   대신하므로, 지우면 화면을 옮길 방법이 없어진다. 이 한 줄이 그 규칙의 예산판이다.
  //
  //   ⚠️ 2026-08-15 (재설계 ②) — `state.board` 조건이 한 번 **빠졌었다.** 2026-08-14 에는
  //   전술판만 헤더를 걷었고 드릴 편집은 제목·[저장]·[시연] 때문에 남겼는데, 그 셋이 각각
  //   인스펙터·기능 바·트랜스포트로 가면서 남길 이유가 없어졌다(코트 크기 문제 — 헤더 48 이
  //   남으면 기둥이 2열로 흘러 폭에서 44 를 더 먹는다).
  //
  //   ⚠️ 2026-08-20 (기현님 지시) — `state.board` 조건이 **돌아온다.** 이번엔 반대 방향이다:
  //   드릴 편집이 시연과 같은 헤더를 쓰도록 다시 얻는다("두 화면이 비슷한 레이아웃이어야
  //   ux가 좋아진다"). 그래서 이제 "넓은 창에서 헤더가 없다"는 **자유 전술판에만** 남는
  //   예외다 — `state.board && !state.narrow`(넓은 전술판)일 때만 0, 그 밖은 전부 48
