...
export function AppHeader({
  ...
  return (
    ...
    <header style={{ ...HEADER_STYLE, padding: headerPadCss(navInHeader || !!config.compact) }}>
      ...
      {/* ⚠️ 2026-09-19 — **세 칸 균등(`1 1 0`)을 그만둔다.** 옛 모양은 세 칸이 정확히 1/3 씩이라
          1133px(레일 뺀 헤더 1049)에서 각 칸이 **324px 로 고정**됐고, 그것이 두 결함의 **한 원인**이었다:
            · 자유 전술판 — **빈 왼쪽 칸이 324 를 붙들어** 부제가 359 를 못 받아 말줄임됐다(실측).
            · 드릴 목록 — 오른쪽 칸도 324 라 검색칸(210)+[새 드릴]이 안 들어가 **헤더가 두 줄**(74)이 됐다.
          이제 **제목 칸만 내용 크기**(`0 1 auto`)로 두고 양옆은 `1 1 0` 그대로 둔다.
          **가운데 정렬은 그대로 지켜진다** — 양옆이 같은 flex 라 남는 폭을 똑같이 나눠 가지므로
          제목의 중심이 언제나 `X + T/2 = (W−T)/2 + T/2 = W/2` 다(칸이 1/3 일 필요가 없었다).
          옛 주석의 걱정(긴 부제가 basis 로 헤더를 접는다)은 남지만 **뜻이 달라진다**: basis 가 이제
          내용 폭이라, 접히는 순간은 «너무 일찍» 이 아니라 «정말로 안 들어갈 때» 다 — 그건 이 헤더가
          원래 원하던 동작이다(HEADER_STYLE 의 *"잘라 없애느니 두 줄로 흐르게 둔다"*).
          `minWidth: 0` 은 그대로라 끝까지 밀리면 칸 안에서 말줄임된다. */}
      <div style={{ minWidth: 0, flex: absolute ? (symmetric ? '0 1 auto' : '1 1 6rem') : '1 1 12rem' }}>
        ...
        {config.search && (
          <label
            ...
            style={{
              ...
              color: 'var(--faint-text)',
              fontSize: '0.8125rem',
              // ⚠️ 2026-09-19 — `flex` 와 `minWidth` 를 함께 건다. 옛 모양은 `minWidth: 210` 뿐이라
              //    **하한만 있고 상한이 없었고**, 안쪽 `<input>` 의 브라우저 기본 크기(size=20)가
              //    칸을 **301px** 까지 밀어 올렸다(실측). 그 301 + [새 드릴] 115 가 오른쪽 칸에 안
              //    들어가 헤더가 두 줄(74)로 접혔다 — 210 을 지키려던 하한이 정작 지킨 것은 301 이었다.
              //    `1 1 210px` 은 «210 을 기준으로 남는 만큼 늘고, 모자라면 210 까지 줄어든다» 다.
              flex: '1 1 210px',
              minWidth: 210,
            }}
          >
            ...
            <input
              ...
              style={{
                ...
                font: 'inherit',
                width: '100%',
                // `width:100%` 만으로는 부족하다 — flex 자식의 자동 최소 크기(min-width:auto)가
                // `<input>` 의 **기본 size** 를 바닥으로 깔아, 부모가 줄어도 이 칸이 안 줄어든다.
                minWidth: 0,
              }}
            />
          </label>
