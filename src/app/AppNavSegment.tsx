          ...
          return (
            <button
              ...
              style={{
                ...
                //
                // ⚠️ 2026-09-19 — **여기는 `--hit-slim`(29)을 안 쓴다.** 헤더 오른쪽 묶음은 그날
                //    지시로 29 로 내렸는데(AppHeader 의 `--hit` 덮개), 이 세그먼트는 44 그대로다.
                //    이유 둘: ① 지시가 *"헤더의 **오른쪽** 각 버튼들"* 이었다 ② 이 칸은 세로 화면에서
                //    **유일한 이동 수단**이다 — 오른쪽 묶음의 [공유]·[테마]는 못 누르면 불편하지만,
                //    이걸 못 누르면 화면을 바꿀 길이 없다. 그리고 세로 화면은 세로 공간이 남는 쪽이라
                //    (판이 서고 트레이가 기둥이 된다) 44 를 지키는 값이 싸다.
                //    그래서 세로 헤더는 48(44+여백 2·2)이고 가로 헤더는 33 이다 — 어긋남이 아니라
                //    **각 방향에서 귀한 것이 다르다**는 판단이다.
                minHeight: 'var(--hit)',
                minWidth: 'var(--hit)',
                padding: '0 0.5rem',
