...
describe('hitRadius', () => {
  ...
  it('네 종류 모두 상한표(§6.5)와 일치한다', () => {
    // note 22: §4.3 P1-2 로 12.5 에서 올렸고, §4.3 P1-5 로 그 값이 **선택 링 반지름과 같아야
    // 한다**(`NOTE.ringRadiusPx`) — 메모가 실제로 그려지는 쪽지가 되면서 상한이 링보다 작으면
    // "링 안을 눌렀는데 안 잡힌다" 가 되기 때문이다. 그 동치는 objects/notePaper.test.tsx 가
    // 붙잡는다. physics/hitTest.ts 의 복제본과도 같은 값이어야 한다.
