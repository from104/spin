...
describe('FullCourtLines', () => {
  ...
  it('editor: 골 십자 2개는 그리되 골대 원은 그리지 않는다 (§5.4)', () => {
    ...
    // §5.4 는 하프에서 한 번 더 돈다 — 골대 원도 흰 센터 점도 없다.
    const half = renderCourt('half', 'editor');
    expect(half.querySelectorAll('g[fill="#f5f5f5"] > circle')).toHaveLength(0);
    expect(half.querySelector('circle[fill="#ffffff"]')).toBeNull();
  });
});
...
describe('센터 마크 — 코트를 그리는 다섯 화면 전부에 닿는다', () => {
  ...
  it('⑤ 썸네일은 실제로 full·half 두 판 모두에 센터 마크를 그린다', () => {
    ...
    // 크기 축도 같이 돈다 — size prop 을 빼먹으면 25×14 썸네일에 30×18 의 X 가 뜬다
    // (§6.4 가 실제로 겪은 사고의 형태다).
