...
/** 네이티브 셸 빌드에만 주입되는 **웹앱의 공개 출처**(예 `https://spin.atit.app`). 없으면 undefined.
 *
 *  **왜 필요한가**: 네이티브 셸의 출처는 웹앱의 도메인이 아니다 — 데스크톱은 `tauri://localhost`,
 *  안드로이드는 `https://localhost` 다. 그래서 ① 상대 경로 `/api/share` 가 아무 데도 닿지 않고,
 *  ② `location.origin` 으로 지은 링크는 받는 쪽이 열 수 없다(2026-09-13 기현님 데스크톱 실기 —
 *  "서버에 닿지 못했습니다"). 둘 다 이 값으로 푼다.
 *
 *  ⚠️ 2026-09-17 — `desktopWebOrigin()` 에서 이름과 읽는 칸이 넓어졌다(PLAN-ANDROID 결정 19).
 *  셸이 둘이 되면서 같은 뜻의 값이 둘(`SPIN_DESKTOP_WEB_ORIGIN`·`SPIN_ANDROID_WEB_ORIGIN`)이
 *  됐는데, 한 빌드에 둘이 같이 들어오는 일은 없다 — `vite.config.ts` 의 envPrefix 가 대상별로
 *  한쪽만 연다. 그래도 순서를 정해 두는 이유는 그 게이팅이 틀렸을 때 **조용히** 엇갈리지 않게
 ...
 *  (결정 3 도메인 독립). `VITE_SHARE_API_BASE` 가 있으면 그것이 이기고, 없으면 네이티브 출처
 *  (`nativeWebOrigin()`) 아래 `/api/share`, 그것도 없으면 상대 경로다. 뒤 슬래시는 붙어 있어도
...
export function shareApiBase(): string {
  ...
  const native = nativeWebOrigin();
  const base = explicit ?? (native ? `${native}/api/share` : '/api/share');
  return base.replace(/\/+$/, '');
}

/** 링크 앞에 붙는 출처. 웹은 `location.origin`(앱이 있는 바로 그 도메인), 네이티브 셸은
 *  `nativeWebOrigin()` — `tauri://localhost/s/…` 도 `https://localhost/s/…` 도 아무도 못 연다.
 *  안드로이드에서 이 값이 맞아야 App Links 가 받아 갈 링크가 나온다(PLAN-ANDROID 결정 9·19). */
export function shareLinkOrigin(): string {
  return nativeWebOrigin() ?? window.location.origin;
}
