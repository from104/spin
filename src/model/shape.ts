 ...
 *
 *  ⚠️ 2026-09-14 — 0.13 → **0.156**(기현님 지시 *"도형 색 20% 더 불투명하게"*, 1.2배).
 *  옛 주석의 실측표 *"0.13 이면 … 한 겹 1.19:1 · 두 겹 1.41:1 · 세 겹 1.68:1"* 은 **틀린
 *  값이었다** — 다시 재면 0.13 에서 흰색이 1.29/1.59/1.91 이다(옛 수는 α 0.09~0.10 의 것이다).
 *  지우지 않고 남기는 이유는 «이 표를 믿고 값을 고르지 마라» 는 표시다.
 *
 *  0.156 의 실측(코트 #1f7a46 위, 한 겹/두 겹/세 겹):
 *    흰 1.35 / 1.74 / 2.12 · 노랑 1.26 / 1.53 / 1.80 · 하늘 1.16 / 1.31 / 1.46 · 주황 1.02 / 1.07 / 1.14
 *  단계가 벌어지는 성질(겹칠수록 진해진다)은 그대로고, 경계를 보는 일은 면이 아니라
 *  **검정 케이싱**이 진다(3.93:1) — 그래서 면을 이만큼 올려도 «어디까지가 이 도형인가» 는 안 흐려진다. */
export const SHAPE_FILL_OPACITY = 0.156;
