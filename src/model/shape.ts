 ...
 *
 *  ⚠️ 2026-09-15 — 0.156 → **0.30**(기현님 지시 *"도형 불투명도를 30%로 상향"*). 하루 전
 *  0.13 → 0.156(*"20% 더 불투명하게"*, 1.2배)으로 올렸던 것을 실기에서 보고 다시 올린 것이다.
 *  두 번 다 방향이 같으니 **낮게 잡는 것이 계약**이라던 위 문장의 «낮게» 는 실기에서 한 번도
 *  맞은 적이 없다 — 다만 상한은 여전히 있다: 세 겹이 코트를 덮어 판이 흰 종이가 되면 안 된다.
 *  0.30 에서 흰색 세 겹이 3.26:1 로 **비텍스트 하한 3:1 을 처음 넘는다** — 여기가 그 상한의
 *  문턱이고, 다음에 더 올리자는 말이 나오면 이 줄이 «세 겹부터는 코트가 아니라 도형이 바탕이
 *  된다» 는 뜻이라는 근거다.
 *
 *  실측(코트 #1f7a46 위, 한 겹/두 겹/세 겹):
 *    0.156 — 흰 1.35/1.74/2.12 · 노랑 1.26/1.53/1.81 · 하늘 1.16/1.32/1.47 · 주황 1.02/1.07/1.14
 *    0.30  — 흰 1.78/2.57/3.26 · 노랑 1.56/2.12/2.60 · 하늘 1.34/1.62/1.86 · 주황 1.08/1.22/1.37
 *  단계가 벌어지는 성질(겹칠수록 진해진다)은 그대로고 — 오히려 간격이 넓어져 겹침이 더 잘
 *  읽힌다 — 경계를 보는 일은 면이 아니라 **검정 케이싱**이 진다(3.93:1). 그래서 면을 올려도
 *  «어디까지가 이 도형인가» 는 안 흐려진다.
 *
 *  ⚠️ 옛 주석의 실측표 *"0.13 이면 … 한 겹 1.19:1 · 두 겹 1.41:1 · 세 겹 1.68:1"* 은 **틀린
 *  값이었다**(0.13 을 다시 재면 흰색 1.29/1.59/1.91 — 옛 수는 α 0.09~0.10 의 것이다). 지우지
 *  않고 남기는 이유는 «이 표를 믿고 값을 고르지 마라» 는 표시다. */
export const SHAPE_FILL_OPACITY = 0.3;
