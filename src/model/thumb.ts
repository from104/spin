...
export interface ThumbSpec {
   ...
   *  담는다 — ⚠️ 2026-09-14 로 이 문장은 거짓이 됐다. 옛 문장은
   *  *"도형의 색은 사용자 데이터가 아니라 상수 하나(`SHAPE_COLOR`)라 이 파일 머리말의 «색을 굽지
   *  않는다» 에 걸리지 않는다"* 였는데, 이제 도형도 `color` 를 갖는다(기현님 지시, 흰·하늘·노랑·주황).
   *  다만 **결론은 안 바뀐다**: 도형은 모델 객체를 `structuredClone` 으로 통째로 담으므로 색이
   *  첨자가 아니라 hex 로 저절로 실린다 — 메모(`NoteLabel.color`)와 같은 선례다. 첨자로 굽는
   *  규약(`ARROW_COLORS` 의 c)은 화살표 전용이고, 여기는 그 길을 안 탄다. 그래서 `SUMMARY_BUILD`
   *  도 안 올린다: 이 기능 전에 저장된 요약의 도형은 **전부 흰색**이라 «없음» 이 참인 기본값이다. 그래서 썸네일이 `ShapeLayer` 를 그대로 재사용하고,
