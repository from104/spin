...
export const DRILL_MIGRATIONS: DocMigration[] = [
  ...
  },
  {
    from: 10,
    to: 11,
    describe: 'drill v10→v11: 개체 표시 순서(zOrder) — 목록이 없으면 지금까지의 고정 순서 그대로다',
    // ⚠️ **적을 참말이 없는 상승이다**(v3→v4·v9→v10 과 같은 형태 — drill.ts 의
    // CURRENT_DRILL_SCHEMA v11 주석). 여기서 전 스텝에 `zOrder` 를 찍으면 **거짓말이 된다**:
    // 목록이 있다는 것은 "사용자가 순서를 정했다" 는 뜻이고(`sceneOrder` 규칙 ③이 그 위에
    // 선다 — 목록이 있는 스텝에서는 나중에 놓은 개체가 기본층 자리가 아니라 맨 위로 간다),
    // 옛 드릴은 아무도 순서를 정한 적이 없다. 빈 배열도 마찬가지다 — `validate` 가 빈
    // 목록의 키를 지우므로 저장본에 표현이 둘이 되기만 한다.
    //
    // 목적은 **도장**이고, 도장의 목적은 옛 앱이 새 파일을 정직하게 거절하게 하는 것이다:
    // v10 앱은 `steps[].zOrder` 를 모르므로 사용자가 정한 순서를 통째로 무시하고 고정 순서로
    // 그린다 — 파일은 멀쩡히 열리고 아무 경고 없이 **다른 그림**이 나온다.
    migrate: (doc) => doc,
  },
];
