...
describe('rules — 3 m 링(2-on-1)', () => {
  ...
  it('[예외 ②] 상대가 3 m 안에 없으면 같은 팀이 몇이든 반칙이 아니다', () => {
    ...
    expect(ringViolation(BALL, [...mates, actor('away', at(REACH_BACK + 10), 260)], [GZ])).toBe(0);
    // 대조군: 그 상대를 링 안으로 한 걸음 들이면 곧바로 켜진다.
    expect(ringViolation(BALL, [...mates, actor('away', at(REACH_BACK - 10), 260)], [GZ])).toBe(TEAM_BIT.home);
  });
  ...
  it('★ 경계: 차체가 3 m 선에 **닿으면** 안이고, 0.1 px 밖이면 밖이다', () => {
    ...
    const judge = (x: number, theta: number): number => ringViolation(BALL, [actor('home', x, 260, false, theta), mate, foe], [GZ]);
  ...
  it('거리는 유클리드다 — 축이 아니라 반지름으로 잰다', () => {
    ...
    expect(ringViolation(BALL, [corner(60), ...others], [GZ])).toBe(0);
    // 대조군: 같은 축 좌표를 50 으로 줄이면 반지름 70.7 로 안이다(부재 단언이 헛것이 아니다).
    expect(ringViolation(BALL, [corner(50), ...others], [GZ])).toBe(TEAM_BIT.home);
  });
...
describe('rules — 판정은 점이 아니라 차체 사각형이다', () => {
  ...
  const judge = (x: number, y: number, theta: number): number =>
    ringViolation(BALL, [actor('home', x, y, false, theta), mate, foe], [GZ]);
...
describe('rules — 골키퍼 면제는 차체가 **조금이라도** 걸치면 성립한다', () => {
  ...
  const mate = actor('home', ball.x + 20, ball.y);
  const foe = actor('away', ball.x + 10, ball.y);
  const withGk = (gk: RuleActor): number => ringViolation(ball, [gk, mate, foe], [GZ]);
  ...
  it('ⓒ 모서리만 닿음(45° 차체의 꼭짓점 하나) → 면제', () => {
    ...
    expect(ringViolation(nearBall, [gk, actor('home', nearBall.x + 10, nearBall.y), actor('away', nearBall.x + 5, nearBall.y)], [GZ])).toBe(0);
    ...
    expect(ringViolation(nearBall, [gkOff, actor('home', nearBall.x + 10, nearBall.y), actor('away', nearBall.x + 5, nearBall.y)], [GZ])).toBe(TEAM_BIT.home);
  });
  ...
  it('★ 기현님 시나리오 — 골 지역에 걸친 골키퍼 + 수비 1명이면 **거짓 2-on-1 이 사라진다**', () => {
    ...
    // ③ 새 판정은 깨끗하다.
    expect(ringViolation(b, [gk, back, striker], [GZ])).toBe(0);
    // ④ 대조군 — 그 골키퍼를 존에서 **완전히** 떼면 곧바로 붉어진다(면제만 사라진 것이다).
    const gkOut = actor('home', GZ.x + GZ.w + BACK + 0.1, b.y, true);
    expect(ringViolation(b, [gkOut, back, striker], [GZ])).toBe(TEAM_BIT.home);
  });
    ...
    for (const mode of COURT_MODES) {
      for (const size of COURT_SIZES) {
        ...
        if (!zone) {
          ...
          const b = { x: 200, y: 200 };
          expect(
            ringViolation(b, [actor('home', b.x + 10, b.y, true), actor('home', b.x - 10, b.y), actor('away', b.x + 20, b.y)], def.ruleZones),
            tag,
          ).toBe(TEAM_BIT.home);
          continue;
        }
        ...
        expect(ringViolation(b, [gk, actor('home', b.x + 20, b.y), actor('away', b.x + 10, b.y)], def.ruleZones), tag).toBe(0);
        // 대조군 — 존에서 완전히 떼면 붉다.
        const gkOut = actor('home', zone.x + zone.w + BACK + 0.1, b.y, true);
        expect(ringViolation(b, [gkOut, actor('home', b.x + 20, b.y), actor('away', b.x + 10, b.y)], def.ruleZones), tag).toBe(TEAM_BIT.home);
      }
    }
  });
});
...
describe('rules — 골 지역 3인', () => {
  ...
  it('존 밖의 선수는 세지 않는다', () => {
    ...
    expect(zoneViolation(GZ, nudged)).toBe(TEAM_BIT.home);
  });
  ...
  it('★ 옛 좌표(피벗이 1 px 밖)는 이제 **안**이다 — 차체가 걸쳐 있기 때문', () => {
    ...
    expect(zoneViolation(GZ, old)).toBe(TEAM_BIT.home);
  });
  ...
  it('경계선 위는 안이다 — 그리고 차체가 변에 **닿기만** 해도 안이다', () => {
    ...
    expect(zoneViolation(GZ, touch(0))).toBe(TEAM_BIT.home);
    expect(zoneViolation(GZ, touch(-0.01))).toBe(0);
  });
});
