...
describe('§5.1 마이그레이션 — 옛 드릴은 30×18 로 못박혀 올라온다', () => {
  it('courtSize 를 모르는 v2 문서가 v3 로 오르며 30×18 을 얻는다', () => {
    ...
    // v2 문서는 v3(코트 크기)·v4(도형)·v5(자유 삼각형)·v6(진영)·v7(선 통일) 다섯 단계를 지난다.
  ...
  it('무손실 — v2 의 모든 키가 값째로 살아남는다', () => {
    ...
    for (const [k, v] of Object.entries(raw)) {
      if (k === 'schemaVersion') continue; // 도장만 2 → 3
  ...
  it('v1 → v7 전 체인이 한 번에 돈다 (교육 필드 + 코트 크기 + 도형 + 자유 삼각형 + 진영 + 선 통일)', () => {
    ...
    // 멱등 — 이미 v7 인 문서를 다시 넣으면 아무 일도 안 한다.
