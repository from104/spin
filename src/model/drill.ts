...
export interface DrillStep {
  ...
  cut?: true;
  // ── 딜레이 없는 연결 (2026-09-08 기현 지시 · docs/PLAN-STEP-LINK.md 결정 1·2) ─────────────
  // *"드릴 편집에서 스텝을 연결하는 옵션을 '끊김, 딜레이 연결, 딜레이 없는 연결' 을 둬서
  //  실질적인 애니메이션 수준의 원칙적으로 만들 수 있도록"* — 셋 중 둘은 이미 있었다(키 없음
  // = 딜레이 연결, `cut` = 끊김). 이 키가 셋째다: 이 스텝의 트윈이 **구간 전체**를 차지해
  // 정지 없이 다음 스텝의 트윈으로 넘어간다(§3.6 sampleDrill). 여러 스텝을 이으면 그대로
  // 키프레임 애니메이션이 된다.
  //
  // ⚠️ **`cut` 과 배타다 — 둘 다 있으면 `cut` 이 이긴다**(끊김이 더 강한 뜻이고, 끊긴 경계에
  // 트윈 길이를 정하는 것은 애초에 뜻이 없다). 정화는 `validate.ts` 가 한다(`seamless` 를
  // 버린다). 저장은 `cut` 과 같은 관행이다: **예외만, `true` 만**(`seamless: false` 는 저장
  // 하지 않는다 — 키 없음이 곧 딜레이 연결이다).
  //
  // 스키마 도장을 **안 올린다**(v11 유지) — `cut` 선례와 같은 논법이다. 스텝의 정지 포즈는
  // 한 픽셀도 안 바뀌고 구간 안에서 트윈을 어떻게 나누느냐만 다르므로, 이 키를 모르는 옛 앱은
  // 같은 그림을 딜레이 연결로 재생할 뿐 **다른 그림을 그리지 않는다**(courtSize 류의 위험 없음).
  seamless?: true;
  // ── 표시 순서 (z-order, 2026-09-06 기현 지시 · docs/PLAN-Z-ORDER.md 결정 1) ───────────────
  // *"각 개체가 겹칠 때 어느것이 뒤에표시되고 앞에 표시되나."*
  //
  // ⚠️ **위 `locked`/`ignored`/`cut` 과 같은 부류다** — "그 스텝의 판이 어떤 상태인가". 그래서
  // 저장 방식도 그쪽 규약을 그대로 따른다: 개체마다 `z?: number` 를 달지 않고 **스텝이 id
  // 목록 하나를 쥔다.** 개체마다 달면 저장 방식이 둘이 되는데(휠체어·공·콘은 cast 정의 +
  // 스텝 좌표라 PoseMap 의 값이 `Vec2`·`StoredChairPose` 라서 필드를 얹을 자리가 없고,
  // 화살표·메모·도형·획만 객체 자체가 스텝 배열이다), 순서를 읽는 코드가 매번 "이건 어느
  // 쪽이지" 를 물어야 한다. 목록 하나면 전부 같은 질문이다.
  //
  // **아래→위**다(배열 앞이 뒤쪽/먼저 그려지는 것). 7종 id 를 한 목록에 섞는다 — 접두
  // (`sh cn fh ar ch bl nt`)가 종류를 구별하므로 종류별로 목록을 나눌 이유가 없고, 나누면
  // "도형과 화살표 중 뭐가 위냐" 를 목록 밖에서 또 정해야 한다.
  //
  // ⚠️ **없으면 §3.5 기본층 순서 그대로다** — 옛 드릴은 한 픽셀도 안 변한다. 첫 재배치 때
  // 그 스텝의 **전체 순서를 물질화**한다(부분 목록은 "목록에 없는 것" 의 자리를 매번 다시
  // 정해야 해서 읽는 규칙이 둘로 갈린다). 목록을 푸는 유일한 자리는 `model/zOrder.ts` 의
  // `sceneOrder` 이고, 판·시연·인쇄·PNG·썸네일 다섯 렌더 경로와 히트테스트가 전부 그
  // 한 함수를 읽는다(경로별 드리프트가 이 저장소에서 네 번 터졌다).
  //
  // 고아 id(개체가 지워졌는데 목록에 남은 것)는 `sceneOrder` 가 무시하고 `validate.ts` 의
  // `sanitizeZOrder` 가 지운다. 개체 제거는 `edits.ts` 의 `stripStepFlags` 를 지난다.
  zOrder?: string[];
}
...
/** v11 = 개체 표시 순서(`DrillStep.zOrder`) — 2026-09-06 기현 지시
 *  (*"드릴 편집에서 객체들의 표시 레이어조정 … 각 개체가 겹칠 때 어느것이 뒤에표시되고 앞에
 *  표시되나"*). 정본 계획서는 `docs/PLAN-Z-ORDER.md`(결정 3).
 *
 *  ⚠️ **②를 넘는다**(도장을 올린다). v4(작도 도형)·v10(획)과 **같은 형태의 판단**이다:
 *   ① 없으면 기본층 순서 — 전역이다. 마이그레이션이 적을 참말은 없다(v3→v4·v9→v10 과 같다).
 *   ② ✗ **순서는 문서 내용이다.** v10 앱은 `steps[].zOrder` 를 모르므로 그 목록을 통째로
 *      무시하고 **고정 순서**로 그린다 — 파일은 멀쩡히 열리고 아무 경고 없이 **다른 그림**이
 *      나온다. 이것은 v10(획)의 "빠뜨린다" 보다 오히려 courtSize(v3)에 가깝다: 빠지는 것이
 *      아니라 **저장된 것의 해석이 바뀐다.** 압박 구역 도형을 일부러 칩 위로 올려 가린 판이
 *      옛 기기에서는 도형이 칩 밑에 깔린 판으로 열린다 — 코치가 가리려던 것이 그대로 보인다.
 *   ③ 대가는 그대로다 — 배포된 옛 빌드가 새 파일을 too-new 로 거절한다. ②가 성립하는 한
 *      **거절이 정답**이고, 그것이 이 상승의 목적이다.
 *
 *  즉 v4·v10 과 마찬가지로 「⚠️ 적을 참말이 없는 상승」이지만, 도장 자체가 목적이다. */
export const CURRENT_DRILL_SCHEMA = 11;
