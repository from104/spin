...
export type EditorAction =
  ...
  // 자유 전술판 전용 — **코트 갈아끼우기**. 히스토리를 쌓지 않고 비운다. 코트가 바뀌면 옛
  // 배치는 어떤 아핀 변환으로도 되살릴 수 없으므로(D12), 되돌릴 과거가 있다고 말하는 쪽이
  // 거짓말이다.
  //
  // ⚠️ **[비우기]는 2026-08-28 부터 이 액션을 쓰지 않는다**(기현님 지적). 그때까지 비우기가
  //    여기로 온 이유는 게이트였다: 게이트가 `past.length === 0` 를 "판이 비었다" 의 대용으로
  //    썼으므로, 비우기가 COMMIT 이면 비우자마자 코트 전환이 잠겼다. 게이트가 판 위 개체를
  //    직접 세게 된 지금 그 사슬은 없고, 비우기는 되돌릴 수 있는 DRILL_LOAD 로 간다
  //    (BoardScreen.onReset · EditorWorkspace 의 게이트 주석).
