...
export const ja: Record<DictKey, string> = {
  ...
  'help.editor.item5.desc': '右の機能バーの［情報］（鉛筆アイコン）で種類・状況・説明・チーム名を編集します。実演画面の同じボタンは目のアイコンで読み取り専用です。',
  ...
  'help.present.item4.desc': '右の機能バーの［情報］（目のアイコン）がドリルの種類・状況・説明を表示します。実演中は読み取り専用です — 編集は編集画面の同じボタン（鉛筆アイコン）から。',
