...
export type ToolId =
  ...
  | 'shapeTriangle'
  | 'shapeRect'
  // 2026-09-03 — 지우기 도구가 **되살아났다**(기현 지시). 2026-08-16 에 없앤 그것과 같은
  // 이름이지만 같은 물건이 아니다: 옛것은 선택과 겹치는 **드래그** 도구였고 지금 것은
 ...
 *
 *  [A-2] `tool === 'select'` 에서만 돈다(§4.3 P1-2)
 *  무조건 돌리면 **집는 것과 놓는 것의 판정이 같아진다** — 배치 도구에서 44 CSS px 안에
 *  뭔가 있으면 콘을 나란히 못 놓게 된다. 관대함은 '고르기' 의 성질이지 '만들기' 의 성질이
 *  아니다. (2026-08-16 이전에는 지우개 도구도 이 문단의 근거였다 — 관대한 반경으로 조용히
 *  지워 버리는 일을 막는 것. 그 도구는 사라졌고 삭제는 선택 후 Delete 로 모였다.)
 *  🔁 **2026-09-03 — 그 근거가 되살아났다.** `eraser` 가 돌아왔으므로 이 가드는 다시 두 몫을
 *  한다: 배치 도구가 나란히 놓을 수 있게 하는 것과, **파괴 도구에 관대함을 안 주는 것**.
 *  허용 목록(`=== 'select'`)이라 새 도구는 자동으로 엄격 패스만 탄다 — 그것이 이 형태를
 *  `!== 'place류'` 같은 거부 목록으로 안 쓰는 이유다.
 *
