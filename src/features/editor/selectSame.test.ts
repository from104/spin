...
describe('sameKindGroup — 무엇이 "같은 것" 인가', () => {
  it('휠체어는 **같은 팀**끼리다 — 판 위의 모든 칩이 아니다', () => {
    const g = sameKindGroup('ch_h1', scene());
  ...
  it.each([
    ...
    const g = sameKindGroup(id, scene());
  ...
  it('콘은 색을 안 가린다 — 판 위의 콘 전부가 한 덩어리다', () => {
    expect(sameKindGroup('cn_3', scene())?.ids).toEqual(['cn_1', 'cn_2', 'cn_3']);
  });

  it('혼자뿐이면 null — 눌러도 아무 일 없는 항목은 안 낸다', () => {
    expect(sameKindGroup('nt_1', scene())).toBeNull();
  });

  it('판에 없는 id 는 null — 메뉴가 유령 명단을 만들지 않는다', () => {
    expect(sameKindGroup('ch_none', scene())).toBeNull();
    expect(sameKindGroup('zz_1', scene())).toBeNull();
  });

  // ★ 잠긴 개체는 덩어리로 안 집힌다(고무줄·Ctrl+A 와 같은 규칙). 못 움직이는 것이 섞이면
  //   뒤이은 드래그가 통째로 안 먹는데, 터치에는 그 하나를 빼는 손짓이 없다.
  it('잠긴 것은 빠진다', () => {
    const g = sameKindGroup('cn_1', scene({ locked: new Set(['cn_2']) }));
    expect(g?.ids).toEqual(['cn_1', 'cn_3']);
  });

  it('잠긴 것을 빼고 나서 혼자 남으면 null 이다', () => {
    expect(sameKindGroup('bl_1', scene({ locked: new Set(['bl_2']) }))).toBeNull();
  });

  it('짚은 것 자신이 잠겨 있어도 명단에서 빠진다 — 규칙에 예외를 두지 않는다', () => {
    const g = sameKindGroup('cn_1', scene({ locked: new Set(['cn_1']) }));
