...
//     (§6.1 규칙 1 — React 는 프레임 단위 움직임을 구동하지 않는다). 카드가 손가락을 따라다니지
//     않고 **줄이 갈라지는** 방식인 것도 같은 이유다.
//   · **커밋은 손을 뗄 때 한 번.** 칸을 넘을 때마다 dispatch 하면 되돌리기가 지나온 칸 수만큼
//     쌓여, 한 번 끈 것을 되돌리는 데 여러 번 눌러야 한다.
//
// 2026-08-17 사이드바 이전(PLAN-STEP-EDITING.md 구현 순서 ②) — 소비처가 TransportBar 의 가로
// 칩 줄에서 StepSidebar 의 세로 카드 목록으로 바뀌면서 **드래그 축을 골라 받는다**. 좌표축만
// 다를 뿐 문턱·미리보기·커밋 시점 규칙은 그대로다 — 축을 인자로 빼는 것이 그 규칙을 두 벌
// 적는 것보다 싸다.
...
/** 포인터 사건에서 드래그 축의 좌표만 뽑는다. */
function axisPos(e: Pick<PointerEvent, 'clientX' | 'clientY'>, axis: 'x' | 'y'): number {
...
export interface UseStepReorderDragOptions {
  /** 드래그가 읽는 좌표축. 'x' = 가로 줄(항목이 옆으로 늘어선다), 'y' = 세로 목록(항목이
   *  위아래로 늘어선다). 기본값을 두지 않는 이유는 호출부가 자기 레이아웃 축을 **명시로**
   *  고르게 하기 위해서다 — 기본값이 있으면 세로 목록에서 실수로 가로 축을 그대로 쓰고도
   *  타입은 조용히 통과한다. */
  axis: 'x' | 'y';
  /** 지금 **화면에 보이는 순서**대로의 항목 중심 좌표(드래그 축 기준). 미리보기가 바뀌면
   *  DOM 도 바뀌므로 매번 다시 잰다. */
...
export interface StepReorderDrag {
  /** 끌고 있는 항목과 미리보기 자리. 문턱을 넘기 전에는 null. */
  state: StepDragState | null;
  /** 항목의 onPointerDown 에 그대로 연결한다. */
  start(e: ReactPointerEvent, id: StepId, from: number): void;
  /** 방금 끝난 것이 드래그였는가(=뒤따르는 click 을 삼켜야 하는가). 한 번 읽으면 지워진다. */
  consumeDragClick(): boolean;
}

export function useStepReorderDrag({ axis, measureCenters, onCommit }: UseStepReorderDragOptions): StepReorderDrag {
  ...
  const start = useCallback(
    (e: ReactPointerEvent, id: StepId, from: number) => {
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        const pos = axisPos(ev, axis);
        if (!s.moved) {
          if (Math.abs(pos - s.startPos) < DRAG_THRESHOLD_PX) return;
          s.moved = true;
          setState({ id: s.id, from: s.from, to: s.to });
        }
        const to = dropIndexAt(optsRef.current.measureCenters(), pos, s.to);
