...
describe('③ 크롬 예산이 배치 축과 화면을 안다', () => {
  it('★ 작은 가로 창의 코트 상자와 축척', () => {
    // 944 → 900 → **944**. 2026-08-14 에 [드릴로 저장]이 기능 바로 오며 칸이 12가 됐고,
    // 1024×600 은 좁은 창이라 헤더(52)가 남아 바에 548 만 남는다 — 12칸이 **2열로** 흘러
    // 기둥이 한 열만큼 넓어졌고 코트가 900 으로 줄었다.
    // 2026-08-27 — [골대]·[속도]·[보기]가 [보드 설정] 모달로 들어가 칸이 **9**가 되면서
