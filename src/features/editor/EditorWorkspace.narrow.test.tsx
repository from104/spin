...
describe('narrow === false — PC 경로는 한 바이트도 안 바뀐다', () => {
  ...
  it('넓은 창의 판 DOM 이 현재 기준선과 바이트 동일하다', async () => {
    ...
    //    되살릴 수 있습니다"* 로 바뀌었다(비우기가 실제로 되돌아오게 됐다). diff 는 그 속성값
    //    하나뿐이다 — 뼈대 스냅샷은 안 바뀐다(속성 **값** 변경이라 요소·style 목록이 그대로다).
    expect(createHash('sha256').update(main.outerHTML).digest('hex')).toBe(
      '1a6a0905ede41cb357a5635283aeab01436a7ddcce20dbd0ffcdd774a5bf86c5',
    );
  });
