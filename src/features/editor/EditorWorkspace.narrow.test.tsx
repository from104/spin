...
describe('narrow === false — PC 경로는 한 바이트도 안 바뀐다', () => {
  ...
  it('넓은 창의 판 DOM 이 현재 기준선과 바이트 동일하다', async () => {
    ...
    //    2026-08-27 갱신 — 기능 바에서 **버튼 셋**([골대]·[속도]·[보기])이 빠지고 트레이의
    //    `설명` 손잡이가 [메모] 도구 버튼으로 **치환**됐다(서랍 폐지). diff 는 그 네 자리뿐이고,
    //    코트 `<svg>` 안·헤더·개체 칩은 한 줄도 안 움직인다 — 셋은 [보드 설정] 모달 안으로
    //    들어갔고 모달은 닫혀 있어 DOM 에 없다.
    //    2026-08-28 갱신 ① — [비우기] 칸의 `title` 문구가 *"되돌릴 수 없습니다"* → *"되돌리기로
    //    되살릴 수 있습니다"* 로 바뀌었다(비우기가 실제로 되돌아오게 됐다).
    //    2026-08-28 갱신 ② — 그 [비우기] 칸이 통째로 **[보드 설정] 모달 안으로** 갔다(기현 지시).
    //    기능 바가 9 → 8 칸이 되므로 버튼 하나 + 그 <span> 이 빠진다. 뼈대 스냅샷도 같은 이유로
    //    두 줄이 준다. 코트 `<svg>` 안·헤더·개체 칩은 한 줄도 안 움직인다.
    //    2026-08-29 갱신 — **차체 실측 반영**(1.5×1.0 → 1.3×0.8 m). 이번엔 반대로 코트
    //    `<svg>` **안만** 움직인다: 휠체어 칩의 rect/볼가드 좌표와 트레이 칩 SVG 의 종횡비가
    //    상수에서 파생되기 때문이다(37.5×25 → 32.5×20). 원소 개수와 구조는 그대로라 위
    //    뼈대 스냅샷은 안 바뀌었다 — 그것이 "치수만 바뀌었다" 의 증거다.
    ...
    //    2026-08-31 갱신 — 그 받침판 색이 **짙은 벽돌**(`#5c2310`)이 됐다(기현 지시: *"받침대
    //    색이 너무 이질적이야"*). 기둥 테두리와 같은 계열을 유지한 채 밝은 쪽에서 어두운 쪽으로
    //    뒤집은 것이다. 바뀌는 것은 같은 rect 4장의 `fill` 한 속성뿐이다 — 원소도 좌표도
    //    그대로라, 이 해시만 움직이고 위 뼈대 스냅샷은 가만히 있는 것이 그 증거다.
    //    9.3(2026-09-03) 의 diff 는 **삭제 3줄** — 기능 바 맨 끝의 구분선 + [드릴로 저장] 칸(button
    //    + span). 기현 지시로 보드에 헤더가 돌아오면서 그 칸이 헤더 주 액션 [+ 드릴로 편집]으로
    //    올라갔다. 맨 끝 삭제라 위 칸들의 좌표는 그대로다(§3 불변식 1) — 뼈대 스냅샷 diff 도 그 3줄뿐.
    //    2026-09-03 갱신 — **[지우기] 도구가 되살아났다**(기현 지시). diff 는 **추가 한 칸**:
    //    트레이 기능 구역 맨 끝, [메모] 다음에 ToolButton 하나(button + 아이콘 span + 라벨 span
    //    + 단축키 배지 span)가 붙는다. 맨 끝 추가라 앞 칸들의 좌표는 한 픽셀도 안 움직인다
    //    (§3 불변식 1) — 뼈대 스냅샷 diff 도 그 4줄뿐이고, 코트 `<svg>` 안·헤더·개체 칩은
    //    한 줄도 안 바뀐다. 이 두 사실은 **실검으로 확인했다**: ToolRail.tsx 만 되돌리면
    //    옛 해시(e642f6…)가 그대로 나온다 = 이 해시 변화의 원인이 그 버튼 하나임이 증명된다.
    //    9.4(2026-09-03, 자유 그리기 랜딩) 의 diff 는 **치환 1건**이다. 갱신 전 커밋(d80ab8e)을
    //    `git worktree` 로 따로 풀어 같은 `main.outerHTML` 덤프를 뜨고 전문 diff 로 확인했다 —
    //    hunk 가 하나이고 바뀐 줄이 하나다:
    //     · 트레이 [작도] 서랍 **손잡이의 `title`** 이 `"작도 — 선(L) · 원(O) · 삼각(T) · 사각(R)"`
    //       → `"작도 — 선(L) · 자유(F) · 원(O) · 삼각(T) · 사각(R)"`. 손잡이 title 은 서랍에 담긴
    //       도구를 열거하므로, **버튼이 하나도 안 늘어도** 이 문자열은 움직인다(서랍이 닫혀 있으면
    //       [자유] 버튼 자체는 DOM 에 없다 — 그래서 첫 화면 표적 예산은 36 그대로다).
    //    ⚠️ 여기 잠깐 *"`<defs>` 의 화살촉 마커 id 가 `${uid}-${color}-${kind}` →
    //       `${uid}-${color}${width}-${kind}` 로 바뀌었다"* 가 둘째 항목으로 적혀 있었다. **틀렸다.**
    //       `arrowMarkerWidthKey` 는 기본 굵기(ARROW_STYLE.width)에서 빈 문자열을 내므로 획이 없는
    //       판의 마커 id 는 옛것과 **글자까지 같다** — 위 전문 diff 에 그 줄이 없다는 것이 증거다.
    //       (획을 담은 판에서는 굵기별 마커가 늘지만, 이 스냅샷은 빈 전술판이라 해당이 없다.)
    expect(createHash('sha256').update(main.outerHTML).digest('hex')).toBe(
      'd01ade76c246922419607b3da9babd8eb2ec93a318a4814ceffd81d30283f672',
    );
  });
