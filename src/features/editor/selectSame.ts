...
export function sameKindGroup(id: string, s: SameKindScene): SameKindGroup | null {
  const group = groupOf(id, s);
...
function groupOf(id: string, s: SameKindScene): SameKindGroup | null {
  if (isId(id, 'ch')) {
    ...
    return { label: '같은 팀 전부 고르기', ids: s.chairs.filter((c) => c.team === team).map((c) => c.id) };
  }
  if (isId(id, 'bl')) return { label: '공 전부 고르기', ids: [...s.balls] };
  // 콘은 2색이지만 색으로 가르지 않는다 — 색은 진영 표시로도 쓰고 그냥 구분용으로도 써서
  // "같은 색" 이 무엇을 뜻하는지가 판마다 다르다. 종류로 묶으면 뜻이 하나다.
  if (isId(id, 'cn')) return { label: '콘 전부 고르기', ids: [...s.cones] };
  if (isId(id, 'nt')) return { label: '메모 전부 고르기', ids: [...s.notes] };
  if (isId(id, 'ar')) return { label: '화살표 전부 고르기', ids: [...s.arrows] };
  if (isId(id, 'sh')) return { label: '도형 전부 고르기', ids: [...s.shapes] };
  return null;
}
