...
describe('toolDefs', () => {
  it('§6.10 도구 11종을 레일 순서(포인터→작도→배치→주석→파괴) 그대로 담는다', () => {
    ...
    // 🔁 2026-09-03(2차) — **자유 그리기가 합류해 11종**이다(기현 지시: *"드릴 편집 작도에
    //    자유 그리기 추가"*). 자리는 `line` **바로 뒤**다: 둘 다 '선을 긋는' 도구이고 갈리는
    //    것은 모양이 정해져 있는가뿐이라, [작도] 서랍 안에서 이웃해야 고를 때 견줄 대상이
    //    눈앞에 함께 온다. 서랍 소속이므로 첫 화면 표적 수는 한 개도 안 는다.
  ...
  it('단축키 글자는 keymap 에서 온다 — 이 파일은 키를 정하지 않는다', () => {
    ...
    // 대조군이기도 하다: **자유 그리기(F)는 예외가 아니다.** `freehand` 의 f 가 어느 층에서도
    // 안 쓰이던 빈 자리라 머릿글자 규칙이 그대로 통했다 — 규칙이 통하는데 예외를 만들면
    // 위 셋의 근거("막혔다")가 그때부터 아무것도 안 가른다.
