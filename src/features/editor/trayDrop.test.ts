...
describe('trayDropIntent — 무엇이 될 것인가', () => {
  ...
  it('전부 돌아갈 자리가 있으면 take', () => {
    expect(trayDropIntent(['ch_1'])).toEqual({ kind: 'take', label: '빼기' });
    expect(trayDropIntent(['ch_1', 'bl_1', 'cn_1'])).toEqual({ kind: 'take', label: '3개 빼기' });
  });

  it('돌아갈 자리가 없으면 erase', () => {
    expect(trayDropIntent(['nt_1'])).toEqual({ kind: 'erase', label: '삭제' });
    expect(trayDropIntent(['ar_1', 'sh_1'])).toEqual({ kind: 'erase', label: '2개 삭제' });
  });

  // ★ 고무줄로 칩과 메모를 함께 잡을 수 있으므로(§6.10b) 이 경우는 실제로 생긴다.
  //   한쪽 말로 뭉뚱그리면 둘 중 하나가 거짓이 된다 — 그래서 글자에 양쪽을 다 적는다.
  it('섞였으면 mixed 이고 글자에 양쪽이 다 적힌다', () => {
    expect(trayDropIntent(['ch_1', 'ch_2', 'nt_1'])).toEqual({ kind: 'mixed', label: '2개 빼기 · 1개 삭제' });
  });

  it('빈 명단이면 예고할 것이 없다', () => {
    expect(trayDropIntent([])).toBeNull();
  });
  ...
  it('글자는 메뉴·토스트와 같은 술어를 쓴다 — 하나일 때는 개수를 안 센다', () => {
    expect(trayDropIntent(['ch_1'])!.label).not.toContain('1개');
  });
});
...
describe('trayDropHint — 덮개를 켜고 끈다', () => {
  ...
  it('켜면 종류가 속성으로, 결과가 글자로 나온다', () => {
    trayDropHint.arm(trayDropIntent(['ch_1', 'ch_2']));
  ...
  it('트레이가 없는 화면에서는 아무 일도 안 한다 — 던지지 않는다', () => {
    ...
    expect(() => trayDropHint.arm(trayDropIntent(['ch_1']))).not.toThrow();
    expect(() => trayDropHint.arm(null)).not.toThrow();
  });
});
