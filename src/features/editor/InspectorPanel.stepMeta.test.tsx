...
//   하단 칩으로 스텝을 고른다 → 인스펙터 [스텝] 구역에 적는다 → STEP_META 가 나간다
...
/** 하단 사진 뭉치의 칩(2.10). aria-label 이 `스텝 n: 이름` 이라 이름 반영을 여기서 읽는다. */
const chips = () => screen.getAllByRole('tab');
const chipLabel = (i: number) => chips()[i]!.getAttribute('aria-label');

describe('3.1 스텝 메타 입력 — 칩에서 고르고 인스펙터에서 적는다', () => {
  it('이름·메모를 적으면 STEP_META 가 나가 칩 라벨과 스텝 목록이 즉시 따라온다', async () => {
    const { user } = await openDrillWithInspector();
    // 대조군 — 적기 전에는 기본 이름이고, 그 문장은 화면 어디에도 없다.
    expect(chipLabel(0)).toBe('스텝 1: 스텝 1');
    expect(screen.queryByText('벌려서 받는다')).toBeNull();

    await user.clear(nameInput());
    await user.type(nameInput(), '측면 전개');

    // (1) 칩 라벨 = step.name. **포커스를 떼기 전에** 본다 — blur 를 기다려야 반영된다면
    //     "적는 동안 판이 따라온다" 가 거짓이고, 700ms 병합창도 열리지 않는다.
    expect(chipLabel(0)).toBe('스텝 1: 측면 전개');

    await user.type(noteInput(), '벌려서 받는다');
    // 여기도 아직 textarea 에 포커스가 있는 상태다.
    expect(chipLabel(0)).toBe('스텝 1: 측면 전개');
    // (2) 인스펙터 스텝 목록 = step.note. 두 소비자를 **따로** 찌른다 — 하나만 봐도 통과하면
    //     name/note 중 한쪽만 배선된 상태가 초록불이 된다.
    ...
    expect(screen.getByText('벌려서 받는다', { selector: 'span' })).toBeInTheDocument();
  });
  ...
  it('스텝을 넘기면 입력이 그 스텝 값으로 갈아 끼워진다 — 옛 글자가 남지 않는다 (key={step.id})', async () => {
    ...
    await user.click(chips()[0]!); // ← 1장째로 돌아간다
...
describe('3.1 되돌리기 — 연속 타이핑은 한 칸이다 (COALESCE_TYPES)', () => {
  ...
  it('대조군: 스텝이 다르면 합쳐지지 않는다 — 한 칸은 방금 그 스텝만 지운다', async () => {
    ...
    expect(chipLabel(0)).toBe('스텝 1: 가나');
    expect(chipLabel(1)).toBe('스텝 2: 다라');
