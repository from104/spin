...
export type PrintDoc =
  ...
  /** 팀시트(PLAN-TEAM 결정 18). 코트 그림이 없는 **명단 문서**라 `PrintCourt`·체어 이름 헬퍼를
   *  전혀 쓰지 않는다 — 드릴 시트와 다른 채널이다(TeamPrintSheet.tsx 머리말).
   *  `stripClass` 는 [등급 정보 제외] 토글을 종이까지 그대로 내린 값이다(결정 13·18). */
  | { kind: 'team'; team: Team; stripClass?: boolean };
