...
describe('buildSessionPlan — 표지 표의 한 줄 한 줄', () => {
  it('항목 순서와 번호가 세션 순서 그대로다', () => {
    ...
    const plan = buildSessionPlan(resolved, map);
  ...
  it('제목은 캐시가 아니라 실물 드릴에서 온다 — 종이와 앱이 다른 제목을 말하면 안 된다', () => {
    ...
    expect(buildSessionPlan(resolved, map).entries[0]!.title).toBe('A');
  });

  it('시간 재정의가 캐시를 이긴다', () => {
    const { resolved, map } = sessionOf([drill('A', 10), drill('B', 12)], { overrides: { 1: 20 } });
    expect(buildSessionPlan(resolved, map).entries.map((e) => e.durationMin)).toEqual([10, 20]);
  });

  it('휴식은 없으면 0 이다 — undefined 가 종이에 새지 않게', () => {
    const { resolved, map } = sessionOf([drill('A', 10), drill('B', 12)], { rests: { 0: 5 } });
    expect(buildSessionPlan(resolved, map).entries.map((e) => e.restAfterMin)).toEqual([5, 0]);
  });
  ...
  it('총 시간은 resolveSession 이 계산한 값 그대로다 — 목록과 종이가 갈리면 안 된다', () => {
    ...
    expect(buildSessionPlan(resolved, map).totalMin).toBe(35);
  });
  ...
  it('날짜가 없으면 when 이 undefined 다(표지에서 줄째로 빠진다)', () => {
    ...
    expect(buildSessionPlan(noDate, map).when).toBeUndefined();
  });
});
...
describe('삭제된 드릴 — 표에는 남고 장은 안 만든다', () => {
  ...
  it('참조는 살아 있는데 map 에 안 실린 드릴도 누락으로 본다 — 그릴 코트가 없기는 마찬가지다', () => {
    ...
    const plan = buildSessionPlan(resolved, new Map());
...
describe('formatPlanWhen — 종이에는 날짜가 있어야 한다', () => {
  it('요일·시각 앞에 연월일을 붙인다', () => {
    ...
    expect(formatPlanWhen(new Date(2026, 7, 12, 19, 0).getTime())).toBe('2026-08-12 수 19:00');
  });

  it('대조군: 요일·시각만으로는 지난주 계획서와 구분되지 않는다', () => {
    const a = formatPlanWhen(new Date(2026, 7, 5, 19, 0).getTime());
    const b = formatPlanWhen(new Date(2026, 7, 12, 19, 0).getTime());
    expect(a).not.toBe(b);
  });
});
