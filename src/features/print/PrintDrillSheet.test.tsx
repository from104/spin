...
describe('페이지 수 = 스텝 수', () => {
  it('6스텝 드릴은 6장이다', () => {
    const { container } = render(<PrintDrillSheet drill={drillOf(6)} view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />);
...
describe('드릴 전체 정보는 첫 장에만', () => {
  ...
  it('메타 줄은 난이도·유형·상황·시간이다 — 폐기된 훈련량은 종이에서도 사라졌다 (v8)', () => {
    const r = render(<PrintDrillSheet drill={{ ...drillOf(2), drillType: 'set-piece', situation: 'kick-in' }} view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />);
    ...
    // 상황 미지정이면 그 칸 자체가 없다.
    const bare = render(<PrintDrillSheet drill={{ ...drillOf(2), drillType: 'technical' }} view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />);
