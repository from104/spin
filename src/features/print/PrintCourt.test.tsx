...
describe('PrintCourt — 이 스텝에 놓인 것만, 놓인 자리에 그린다', () => {
  ...
  it('대조군: 빈 스텝이면 개체가 하나도 없다 — "무엇을 넣어도 그린다" 를 막는다', () => {
    ...
    const { container } = render(<PrintCourt drill={drill} step={blank} ariaLabel="코트" view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />);
...
describe('한 문서에 60장이 동시에 있다 — 전역 id 를 쓰면 2장부터 깨진다', () => {
  it('SVG 마다 화살표 마커 id 가 다르다 (§6.6)', () => {
    ...
    const { container } = render(
      <>
        <PrintCourt drill={drill} step={step} ariaLabel="1" view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />
        <PrintCourt drill={drill} step={step} ariaLabel="2" view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />
      </>,
    );
...
describe('진영 표시 — 어느 골을 어느 팀이 지키는가', () => {
  ...
  it('공수 방향을 뒤집으면 종이도 뒤집힌다 — drill.defense 가 실제로 전달된다', () => {
    ...
    const away = render(<PrintCourt drill={{ ...drill, defense: 'away' }} step={step} ariaLabel="코트" view={{ showGrid: true, showGridLabels: true, showRuleZones: true }} />);
