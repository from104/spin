...
//    것이 아니라 **이 드릴이 64 KiB 를 넘는다**는 뜻이라 처방이 "파일로 내보내기" 다
...
export interface ShareLinkModalProps {
  open: boolean;
  /** 링크로 만들 드릴. 카드 진입은 요약이 아니라 **본문**을 저장소에서 읽어 넘긴다. */
  drill: Drill | null;
  onClose(): void;
  returnFocusRef?: RefObject<HTMLElement | null>;
}

export function ShareLinkModal({ open, drill, onClose, returnFocusRef }: ShareLinkModalProps) {
  const titleId = useId();
  const t = useT();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const made = await createShareLink(drill, window.location.origin);
        // ⚠️ 화면보다 토큰이 먼저다(머리말). alive 여부와 무관하게 남긴다 — 모달이 닫혀도
        //    링크는 이미 서버에 만들어져 있고, 토큰이 없으면 그것을 영영 못 지운다.
        rememberShareLink(made.id, { deleteToken: made.deleteToken, createdAt: Date.now(), drillId: drill.id });
