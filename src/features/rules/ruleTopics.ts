...
const TOPICS_KO: readonly RuleTopic[] = [
  // ── 카드 1·2·3 균형 재작성 (2026-09-03, 기현 지시) ──────────────────────────────────
  // *"규칙의 1,2 카드는 너무 밋밋하고 3은 너무 과한 정보가 들어가있다. 세 카드를 균형있게
  //  보기 좋게 전면 재작성. 일단 한국어만"*
  //
  // 진단: 밋밋함의 절반은 **그림이 0장**이라는 것이었다 — 카드 1·2 는 산문만 여섯 덩이씩이고,
  // 카드 3 은 산문 4 + 도해 3 + 장면 2 로 같은 코트를 도해와 장면으로 **두 번** 보여줬다.
  // 그래서 `field-tour`(코트 치수 투어)를 카드 3 → 카드 1 로 옮겼다: 카드 1 은 "이렇게 생긴
  // 경기" 를 그림으로 갖고, 카드 3 은 중복 그림 하나를 던다. 공·장비 도해는 카드 3 에 남는다
  // (RulesScreen.test 가 "선수·코트·공·장비 주제에 공·장비 도해가 붙는다" 를 못박고, 그 제목이
  // 곧 그 카드의 몫이다).
  //
  // 나머지 절반은 리듬이다 — 짧은 단정문이 같은 길이로 늘어서서 어느 문장도 앞에 서지 않았다.
  // 블록마다 첫 줄을 세우고, 세부는 그 뒤에 붙이고, 다른 카드로 가는 다리를 한 줄씩 뒀다.
  //
  // ⚠️ **사실은 한 글자도 새로 넣지 않았다.** 아래 문장들은 전부 2026-08-31 검증(9CARDS §2 의
  //    문장별 근거표)을 통과한 것을 자르고 붙인 것이다. 근거가 바뀌는 문장은 없다 — 그래서
  //    docs/RULES-FIPFA-2025.md 는 손대지 않았다(역순 금지 규율은 사실이 바뀔 때 거는 것이다).
  // ⚠️ 영어·일본어(ruleTopics.en.ts·ja.ts)는 **아직 옛 구조·옛 문안**이다. "일단 한국어만" 이
  //    지시라 그쪽은 다음 회차에 따라간다 — 그때 블록 순서(field-tour 자리)까지 맞출 것.
  {
    key: 'intro',
    title: '파워체어풋볼이란',
    ...
    blocks: [
      ...
      // 2026-09-03 카드 3 에서 옮겨 왔다. 코트 위에 양 팀이 선 한 장면 — 규칙을 설명하기 전에
      // "이렇게 생긴 경기" 를 먼저 보여 준다. 아래 scene-slot(intro-tour)이 원래 이 자리에
      // 바라던 것이 정확히 이것이었다.
      ...
      },
      {
        ...
        note: [
          ...
          '2026-09-03: 그 역할을 지금은 위 `field-tour` 가 한다. 기현님 장면이 오면 field-tour 를 카드 3 으로 되돌리거나 둘 중 하나를 고른다.',
        ],
      },
      {
        kind: 'prose',
    ...
    blocks: [
      ...
      },
      {
        ...
        body: [
          ...
          // '사실상'은 지우면 안 된다 — 위 50.8cm 조항이 '공이 뜨는 일이 있다'를 전제한다.
          // 접촉 문장은 한 문장이어야 한다 — 둘로 가르면 카드 8 이 램핑을 가르치기 전에 "부딪혀도
          // 된다"로 읽힌다(9CARDS §2 카드 2 블록 3).
      ...
      // 2026-09-03 균형 재작성에서 이 블록은 **한 글자도 안 바꿨다** — 표와 1:1 인 문장들이다.
      {
        kind: 'prose',
        heading: '이 경기가 남기는 것',
        body: [
  ...
  },
  {
    ...
    // 튜토리얼 2단계가 배지("도해 3 · 장면 1")를 설명하므로 배지가 실제로 보이는 카드에 건다.
    tutorialAnchor: 'rules-card',
    // 2026-09-03: 이 카드는 **찾아보는 카드**다. 읽는 카드(1·2)처럼 문장을 쌓지 않고, 항목마다
    // 한두 줄 + 그림 하나로 끝낸다. 코트 치수 장면(field-tour)은 아래 court 도해와 같은 말이라
    // 카드 1 로 보냈다.
    blocks: [
      {
        kind: 'prose',
        ...
        body: [
          ...
          '골대는 골라인 한가운데 6m 간격으로 세운 기둥 둘입니다. 그 앞 폭 8m·깊이 5m가 골에어리어, 골라인에서 3.5m 떨어진 점이 페널티 마크입니다.',
        ],
      },
      { kind: 'figure', figureId: 'court' },
      {
        kind: 'prose',
        heading: '선수',
        body: ['한 팀은 최대 4명, 그중 1명은 반드시 골키퍼입니다. 2명 미만이 되면 경기를 시작할 수도, 이어 갈 수도 없습니다.'],
      },
      { kind: 'scene', sceneId: 'lineup' },
      {
        ...
        heading: '공',
        body: [
          '지나치게 튀지 않고, 파워체어가 타고 넘지 못할 정도로 바람을 넣습니다. 규칙이 공에 대해 정하는 것은 이것뿐이라 크기·무게는 규칙에 없습니다.',
      ...
      { kind: 'figure', figureId: 'ball' },
      {
        ...
        heading: '장비',
        body: [
          '바퀴는 4개 이상, 경기 중 최고 속도는 전진·후진 모두 10km/h입니다.',
          '랩 시트벨트·프런트가드·좌우 측면지지대가 필수입니다. 체어의 어느 부분도, 선수의 몸도 체어 바닥판 앞뒤로 튀어나오면 안 됩니다.',
        ],
      },
