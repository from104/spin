...
//
// ── ⚠️ 2026-09-08: 위 문단은 이미 사실이 아니다 ── en·ja 가 각각 `ruleTopics.en.ts`·
// `ruleTopics.ja.ts` 로 완역돼 `RULE_CONTENT_LOCALES`(아래) 가 `['ko','en','ja']` 셋을 안고,
// `ruleTopicsFor(locale)` 도 로케일별로 그 배열을 돌려준다 — "ko 전용"이 죽은 시점은
// 2026-08-31(바로 아래 문단, 규칙 화면 전문 번역 완료)이었는데 이 문단만 갱신을 놓쳤다.
//
...
const TOPICS_KO: readonly RuleTopic[] = [
  ...
  },
  {
    ...
    blocks: [
      // 2026-09-04 기현 지시로 ramming 장면을 지운다 — 옛 두 블록(반칙 목록 + 접촉 허용 범위)이
      // 그 장면 앞에서 램핑을 두 번(짧게 한 번, 자세히 한 번) 정의하고 있었다. 장면이 사라진
      // 자리에서는 그 중복이 그대로 남으므로, 하나로 합쳐 램핑·홀딩·클리핑을 한 번씩만 정의한다
      // (bc2e56a 로 들어온 Law 12 정의 절은 그대로 다 남는다 — 순서만 바뀐다).
      {
        kind: 'prose',
