...
const TOPICS_KO: readonly RuleTopic[] = [
  ...
  //    지시라 그쪽은 다음 회차에 따라간다 — 그때 블록 순서(field-tour 자리)까지 맞출 것.
  {
    ...
    blocks: [
      ...
      // 2026-09-03 셋째 도해(기현님 "카드당 3개는 적절함", PLAN §11.6). 위 산문 '크로스바도 그물도
      // 없습니다' 를 정면도로 — 기둥 둘·6m·없는 자리 둘에 ✕. 축구 골대는 안 그린다(규격이 정본에 없다).
      ...
      // 2026-09-03 도해(워크플로우 채택, PLAN §11). 바로 위 산문의 'PF2 두 명까지' 를 네 칸으로 —
      // 코트·체어·사람 없이 칸과 실선/파선만. 4명 출전이 전제라는 조건은 캡션이 진다(Law 18 :346).
      ...
      // 2026-09-03 도해(PLAN §11). 위 산문이 말하는 '한 벌로 합쳤다' 의 모양 — 네 갈래(프랑스·캐나다/
      // 미국·일본·영국식)가 코임브라에서 영국 규칙을 표준으로 합류해 FIPFA 를 거쳐 이 판까지. 시작
      // 연도는 산문이 이미 말하므로 그림에 없고, 각주가 '간격은 연대 비례 아님' 을 못 박는다.
  ...
  },
  {
    ...
    blocks: [
      ...
      // 2026-09-03 도해(PLAN §11). 위 산문의 전반·하프타임·후반을 **길이에 비례한** 막대로 — '최대
      // 10분' 이 20분처럼 보이면 안 되므로 폭은 LAW 상수에서 파생한다. 진영 교체·멈춘 시간 보상 포함.
      ...
      // 2026-09-03 셋째 도해(PLAN §11.6). 위 산문의 50.8cm 를 골라인 **단면**으로 — 평면 장면(purpose-goal
      // 예약)은 높이를 못 보여 주므로 보완이다. 공 지름·높이 둘 다 상수 파생, 한 축척.
      ...
      },
      {
        ...
        note: [
          ...
          '2026-09-03: 카드당 도해 상한이 3 이 되어(기현님) 50.8cm 단면 도해 `goal-height` 를 위에 넣었다. 그 도해는 높이(단면)만 맡고, 이 장면은 코트 평면에서 굴러 들어감/떠서 넘어감을 움직임으로 보여 준다 — 둘은 보완이지 중복이 아니다.',
        ],
      },
      {
        kind: 'prose',
      ...
      // 2026-09-03 도해(PLAN §11). 위 산문 '공이 체어 사이에 물려… 경기가 막혀 버립니다' 의 규칙 쪽
      // 답 — 물린 채 5초를 넘기면 아웃오브플레이(Law 9 :166). 시간축과 조건 셋만, 체어·원·3m 없음.
  ...
  },
  {
    ...
    blocks: [
      ...
      // 🪦 `figure equipment`(제4조 장비 도해)는 2026-09-03 에 지웠다 — 기현님: *"휠체어 관련 도해는
      //    조악하다. 차라리 빼는 게 낫다."* 속도 막대(전·후진 10km/h 동일) 패널도 같은 도해의 절반이라
      //    함께 갔다. 그 사실은 위 산문 첫 줄이 그대로 말한다.
    ],
  },
  {
    key: 'out-of-play',
