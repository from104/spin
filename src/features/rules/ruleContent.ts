...
const KO_RULE_LAWS: readonly RuleLaw[] = [
  ...
  },
  {
    ...
    title: '제2조 — 공',
    summary: [
      '구형이며 지름 33cm(13인치). 축구공 5호(약 22cm)의 1.5배 크기다.',
      '규칙 본문이 정하는 조건은 공기압 하나다 — 지나치게 튀지 않으면서, 파워체어가 타고 넘지 못할 만큼.',
      '그래서 실물은 저반발·중량형이다. 지름·무게 같은 치수는 Laws 본문이 아니라 FIPFA 장비 규격 소관이다.',
      '경기 중 파열·손상되면 경기를 멈추고, 손상이 발생한 지점에서 세트볼로 재개한다.',
      '킥오프·골킥·코너킥·프리킥·페널티킥·킥인으로 정지해 있는 동안 손상되면 그 재시작을 그대로 다시 한다.',
  ...
  },
  {
    ...
    title: '제4조 — 선수 장비',
    summary: [
      '기본 장비: 팀 통일 저지·하의, 전동휠체어, 랩 시트벨트, 프런트가드, 앞뒤 등번호. 필요한 선수에게는 가슴·어깨·머리 지지대도 필수 장비다.',
      '전동휠체어는 4바퀴 이상이어야 하며(3·4륜 스쿠터 불가), 경기 중 최고 속도는 전진·후진 동일하게 10km/h로 제한된다 — 많은 전동휠체어·스쿠터가 후진 상한을 더 낮게 두는 것과 달리, 이 종목은 양방향 주행이 똑같이 중요해 속도 상한도 같다.',
      '좌우 측면지지대(암레스트)를 양쪽에 달아야 하고, 체어·시트·헤드레스트·몸 어느 부분도 체어 밑판 앞뒤로 튀어나오면 안 된다.',
      '배낭 등 부착 금지(의료 필수 장비는 예외), 다른 체어와 얽힐 날카로운 돌출부 금지.',
      '체어 어느 부분도 공을 가두거나 붙잡도록 만들면 안 되고, 오히려 바퀴가 공을 타넘지 못하게 막는 부착물(전도방지 바 등)을 권장한다.',
    ],
    figureId: 'equipment',
  },
  {
