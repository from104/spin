...
describe('규칙 화면 콘텐츠 ↔ docs/RULES-FIPFA-2025.md', () => {
  ...
  const CORE_FACTS = ['3m', '5m', '3.5m', '8m', '6m', '10km/h', '50.8cm', '20분', '1m',
    // 제2조 — 공(2026-08-21 재작성). 지름은 Laws 본문이 아니라 FIPFA 장비 규격에서 오고,
    // 축구공 5호는 IFAB 규격이다. 둘 다 외부 수치라 여기서 붙잡아 두지 않으면 조용히 어긋난다.
    '33cm', '13인치', '22cm',
    // 🆕 2026-08-31 — 영어 원문을 확보해 정본의 누락 6건을 보강하며 함께 걸었다(bc2e56a).
    // 이 셋을 여기 넣는 이유는 **바로 그 누락이 앱을 틀리게 만들었기** 때문이다: 정본에 없던
    // 사실은 검증에서 "근거 없음"으로 잘려 나갔고(카드 2 의 신체 접촉, 카드 1 의 등급),
    // 아무 테스트도 그 공백을 알려 주지 않았다. 이제 한쪽에서 사라지면 여기서 빨개진다.
    'PF1', 'PF2', '5초', '프런트가드'];
