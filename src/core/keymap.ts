...
const KEY_TO_CODE: Record<string, string> = {
  ...
  // ── 두벌식 자모 → 물리 키 (2026-09-08, PLAN-HELP-OVERHAUL 결정 8) ────────────
  // `code` 가 비는 사건에서만 도는 폴백이므로 정상 경로에는 영향이 0 이다(아래 eventCode 의
  // ⚠️ 2026-09-08 문단이 근거). 표준 두벌식 자판 하나만 싣는다 — 세벌식·옛한글은 자리가
  // 달라 같은 글자가 다른 물리 키에서 나오므로, 여기에 섞으면 한 글자가 두 키를 가리킨다.
  ...
  // Shift 를 쥔 자리. 사건의 `shiftKey` 는 그대로 오므로 여기서 벗겨 낼 것이 없다 —
  // `Shift+C`(콘 색)처럼 Shift 가 뜻인 정의도 자모로 들어왔을 때 그대로 맞는다.
  ㅃ: 'KeyQ', ㅉ: 'KeyW', ㄸ: 'KeyE', ㄲ: 'KeyR', ㅆ: 'KeyT', ㅒ: 'KeyO', ㅖ: 'KeyP',
};
 ...
 *  없는 위험이었다.
 *
 *  ── ⚠️ 2026-09-08: 위 문단의 앞 절반이 뒤집혔다 (PLAN-HELP-OVERHAUL 결정 8) ──────────
 *  `KEY_TO_CODE` 에 두벌식 자모를 실었으므로 **'ㅍ' 는 이제 `KeyV` 로 맞는다.** 뒤집은 이유는
 *  뒷 절반의 전제가 좁았기 때문이다: *"진짜 키보드는 한글 모드에서도 code 를 싣는다"* 는
 *  참이지만, 그때는 이 폴백을 안 타므로 **여기서 문제가 되는 것은 code 가 비는 환경뿐**이다.
 *  그 환경(합성 사건·일부 WebView·화면 키보드·매크로 장치)에서 사용자가 한글 상태이면 `key`
 *  로는 자모만 오고, 그러면 문자 단축키가 **한글 상태에서만** 통째로 죽는다 — 2026-08-16 개편이
 *  없앤 바로 그 증상이 폴백 경로에 남아 있던 것이다. 오발화 위험은 늘지 않는다: 자모는 어차피
 *  글자 입력 칸에서만 나오고, 그 칸은 디스패처의 `isEditableTarget` 관문이 이미 되돌려보낸다.
 *  대가는 두벌식 이외 자판(세벌식)에서 자모와 물리 키가 어긋나는 것 — code 가 오는 정상
 *  경로에는 영향이 없고, 폴백 경로에서만 그 사용자는 개편 전과 같은 상태로 남는다.
 *
