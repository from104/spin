...
export const GOAL = {
  ...
  // 실물 골대는 기둥만 서 있는 것이 아니라 **무게추 사각판** 위에 꽂혀 있고, 그 판은 경기면
  // 반대쪽 — 골라인 밖 + 가까운 사이드라인 쪽 — 으로 비켜 놓여 있다. 그래야 코트 안으로
  // 들어오지 않는다. 화면에도 그대로 그린다(`model/court.ts` 의 `goalBaseRect`).
  //
  // ⚠️ **규정 치수가 아니다.** FIPFA Laws 는 기둥 간격 6 m 와 "파일런 또는 콘" 만 정하고
  //    받침에 대해서는 아무 말이 없다(docs/RULES-FIPFA-2025.md Law 1).
  //
  // ⚠️ **물리 바디가 아니다.** 충돌은 계속 기둥(radiusPx 5)만 한다 — 판까지 막으면 골라인
  //    바깥에서 도는 휠체어가 보이지 않는 벽에 걸린다. 판은 그림이다.
  /** 실물 한 변 — **0.5 m**(기현님 실측). 그리기에는 쓰지 않는다.
   *
   *  ⚠️ 지우지 마라. 이 파일만 보고 아래 표시값 0.75 m 를 실물 치수로 착각하면, 다음 사람이
   *  규정·문서 쪽을 잘못 고친다. 센터 마크가 `CENTER_MARK_SPEC_PX`(규격)와
   *  `CENTER_MARK_HALF_PX`(표시)를 **둘 다** 들고 있는 것과 같은 이유다. */
  baseSpecM: 0.5,
  /** **표시** 한 변 — 0.75 m(18.75 px). 실물의 1.5 배다.
   *
   *  기현 지시 2026-08-30: *"실제는 50cm지만 75cm로 그려."* 30 m 코트를 한 화면에 담으면
   *  0.5 m 는 12.5 px 이고, 그 위에 기둥 원(반지름 4)이 앉으면 판이 기둥의 테두리처럼 읽힌다 —
   *  "골대가 무엇 위에 서 있는가" 가 안 보인다. 공의 `radiusPx`(물리) ↔ `viewRadiusPx`(시각)
   *  괴리와 같은 종류의, **알고 고른** 과장이다. */
  baseSidePx: 18.75,
  /** 기둥이 판 모서리에 정확히 박히면 사진과 다르고 기둥이 판 밖으로 삐져 보인다.
   *  가까운 모서리에서 이만큼 **판 안쪽**에 꽂힌다(0.1 m).
   *
   *  ⚠️ 판을 키울 때 **같이 키우지 않는다.** 이 값이 곧 판이 골라인을 넘어 경기면을 파고드는
   *  깊이이고(기둥이 골라인 위에 서므로), 라인을 덮는 양은 판 크기와 무관하게 작아야 한다.
   *  courtMarks.test 가 그 깊이를 이 상수로 붙잡는다. */
  baseInsetPx: 2.5,
} as const;
 ...
 * **반씩**이 됐다가, 2026-08-30 기현 지시로 **이동:회전 = 2:1** 이 됐다:
 *   · 뒤 2/3 (s ≤ 2/3, 로컬 x ≤ 15.1667) → **그대로 이동**
 *   · 앞 1/3 (s > 2/3)                   → **제자리 회전**
 ...
 * 걸리는 일이 있었다.
 *
 * ⚠️ 2:1 로 넓힌 쪽은 **이동**이다. 회전 띠는 앞 1/3(10.83 px)로 좁아지지만, 회전을 원하는
 *    손짓에는 차체 밖 전방 가이드(towFront)라는 넓은 두 번째 문이 있고 그쪽도 같은 날 지시로
 *    **회전을 우선**하게 됐다(kinematics.stepTow 의 이완 갈래). 반면 이동은 차체를 잡는 길
 *    하나뿐이다 — 문이 하나뿐인 쪽에 넓은 띠를 준다. */
export const DEFAULT_ZONES = {
  sTowRearMax: 0, // s ≤ 0            → towRear  (차체 뒤끝 **밖** = 후방 가이드)
  sSpinMin: 2 / 3, // 0 < s ≤ 2/3      → translate (뒤 2/3 = 그대로 이동)
  // 2/3 < s < 1      → spin      (앞 1/3 = 제자리 회전)
...
/** **무시된(ignored) 휠체어**를 흐리게 그리는 불투명도. 판·시연·PNG·인쇄가 이 값 하나를 읽는다.
 *
 *  왜 이 표시가 편집 전용이 아닌가: 무시는 도구 상태가 아니라 **그 스텝의 사실**이다 —
 *  그 휠체어는 물리 월드에서 통째로 빠져(physics/index.ts) 공이 그냥 통과한다(model/drill.ts 가
 *  `locked`·`cut` 과 같은 부류로 못박는다). 판에서만 흐리면 같은 스텝을 시연·그림·종이로 낸
 *  순간 **물리에서 빠진 선수가 멀쩡한 선수로** 보인다(2026-09-06 경로 대조에서 나온 것).
 *  값 자체는 `ObjectLayer.tsx` 의 리터럴 0.32 였다 — 옮기기만 했다. */
export const IGNORED_OPACITY = 0.32;

/** 코트 배경 사각형의 둥근 모서리(월드 px). **네 경로가 이 값 하나를 읽는다.**
 *
 *  ⚠️ 2026-09-06 — 그 전에는 편집 14 · 시연 16 · PNG 16 · 인쇄 10 으로 **네 자리에 리터럴**이
 *  흩어져 있었고, 어느 것도 왜 그 값인지 적지 않았다(PNG 만 *"PresentStage 와 같은 값"* 이라고
 *  적어 두었는데 그 두 값도 편집 화면과는 달랐다). 정본은 편집 화면이라는 이번 지시에 따라
 *  14 로 모은다 — 썸네일도 원래 14 였다. 종이만 달라야 할 이유가 생기면(재단선 등) 그 사유를
 *  이 줄 아래에 적고 그때 가른다. */
export const COURT_SURFACE_RX = 14;
...
export const INTERACT = {
   ...
   *  2026-08-30 이동:회전 = 2:1 로 **가장 좁은 밴드가 회전(앞 1/3)** 이 됐다:
   *  (1/3)·32.5 = 10.8333 px → 24 / 10.8333 = **2.2154**.
   *  밴드가 좁아질수록 더 크게 확대해야 손가락으로 직접 잡을 수 있다 — 그 아래 배율에서는
   *  터치가 존 핸들(ZoneHandles)만 쓴다. 유도식을 그대로 두고 값만 남겨 두면 주석이
   *  거짓말이 되므로 함께 옮긴다. */
