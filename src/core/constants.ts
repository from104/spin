...
// ── 차체 치수: 실측 반영 (2026-08-29 기현님) ──────────────────────────────────────────
// *"전동휠체어 실측 결과 크기 0.8m, 1.3m였다."* → **1.30 × 0.80 m = 32.5 × 20 px**.
//
// 이것은 **D3 결정과 R1 기각을 뒤집는다.** 그 둘은 1.30 × 0.75 m 안을 물리치고 1.50 × 1.00 m
// 를 골랐는데, 근거는 물리가 아니라 **글자였다**: *"등번호 20 px 글리프가 18.75 px 칩에 안
// 들어가 설계안 1 스스로 폰트 20 → 11 을 요구했고, 그 11 px 는 §7.4 폰트 하한에 걸쳐 저시력
// 사용자에게 판독 불가"*. 그 근거는 **2026-08-11 에 이미 소멸했다** — 기현 지시로 칩 글자가
// `20 × 2/3 = 13.33 px` 로 줄었기 때문이다(ChairChip 의 LABEL_FONT_PX). 13.33 은 20 px 칩에
// 들어가고 하한 11 보다 위다. 즉 실측을 막고 있던 것은 이제 없다.
//
// 잃는 것도 적는다: `hullRadius = 32.5` 라는 **정확한 정수**가 사라진다(D3 가 "부수 이득" 으로
// 적어 둔 것). 대신 hypot(26, 10) = √776 이다. 이득이지 요건이 아니었으므로 실측을 이긴다.
//
// ⚠️ 아래 유도값은 전부 L·W 에서 나온다(§2.5 표). L 이나 W 를 다시 만지면 **여덟 줄을 함께**
//    고쳐야 한다 — 하나만 고치면 그리는 모양과 미는 모양이 갈린다:
//      guardPx = 0.15·L · 25 · pivotToRearPx = 0.2·Lpx · pivotToFrontPx = 0.8·Lpx
//      centroidOffsetPx = 0.3·Lpx · hullRadiusPx = hypot(0.8·Lpx, Wpx/2)
//      rearCornerPx = hypot(0.2·Lpx, Wpx/2) · trackM = W − 0.35(타이어·가드 여유, 양쪽 0.175)
//    그리고 이 파일 밖에도 둘이 있다: `SPIN_RADIUS_MIN_PX`(0.25·Lpx) ·
//    `INTERACT.zoneDirectMinPxPerUnit`(24 / (0.5·Lpx)).
export const CHAIR = {
  lengthM: 1.3,
  lengthPx: 32.5,
  widthM: 0.8,
  ...
  /** = hypot(26, 10) = √776. D3 시절의 정확한 32.5 는 실측과 함께 사라졌다. */
  hullRadiusPx: 27.8567766,
  /** = hypot(6.5, 10) = √142.25 */
  rearCornerPx: 11.9268604,
  /** 전폭 0.80 − 타이어·가드 여유 0.35(양쪽 0.175). **여유는 폭에 비례하지 않는다** —
   *  타이어 두께와 옆 틈은 차가 좁아진다고 같이 얇아지지 않는다. 그래서 비율이 아니라
   *  뺄셈을 유지한다(1.00 − 0.35 = 0.65 였던 것과 같은 식). */
  trackM: 0.45,
 ...
 *  가 다시 생긴다.
 *
 *  ⚠️ 2026-08-17 정정 — **칩 세로는 더 이상 상수가 아니다.** 기현 지시로 메모에 줄바꿈이
 *  생겼고, 줄이 늘면 쪽지도 늘어야 한다. 옛 근거는 *"히트테스트가 보는 장면 스냅샷에는 메모의
 *  좌표뿐이라(§5.12) 반경이 상수일 수밖에 없다"* 였는데, 그 전제 쪽을 고쳤다 — 스냅샷이 이제
 *  메모마다 **칩 반너비·반높이**를 함께 싣는다(`physics/hitTest.ts` 의 notes). 덕분에
 *  긴 메모가 예전부터 갖고 있던 결함("칩은 200 px 인데 픽 원은 22 px")도 함께 사라졌다.
 *
 *  `size`(§3.5 NoteLabel 스키마, 기본 14)는 여전히 글자 크기만 정한다 — 인스펙터·모달이 주는
 *  것은 text·align 둘뿐이라 실제로 size 를 바꾸는 경로는 없다. */
export const NOTE = {
  ...
  chipPadXPx: 7,
  /** 줄 하나가 늘 때마다 칩이 세로로 늘어나는 양(월드 px). 한 줄이면 칩은 정확히
   *  `chipHPx`(24) 라, 줄바꿈이 없는 메모는 2026-08-17 이전과 **한 픽셀도 다르지 않다.**
   *  `size` 에 곱하지 않는 이유는 머리말의 ⚠️ 와 같다 — size 를 바꾸는 경로가 없다. */
  lineHPx: 18,
  /** 칩이 가로로 늘어나는 **상한**. 넘으면 `noteChip.ts` 가 줄을 접는다(자동 줄바꿈).
   *  상한이 필요한 이유는 두 가지다: ① 600자 한 줄이면 칩이 8000 px 가 되어 코트를 덮고,
   *  ② 이제 히트 판정이 칩 상자를 쓰므로 그 칩이 코트의 모든 탭을 삼킨다.
   *  200 은 씨앗 드릴의 가장 긴 메모(≈195 px)가 접히지 않고 들어가는 가장 작은 값이다. */
  chipMaxWPx: 200,
  /** 화면에 그리는 줄 수의 상한. 넘는 줄은 마지막 줄 끝의 '…' 하나로 알린다 —
   *  글을 **버리지는 않는다**(모달을 열면 전문이 그대로 있다). 세로 상한도 이것이 정한다:
   *  `chipHPx + (maxLines − 1) × lineHPx` = 24 + 7×18 = 150. */
  maxLines: 8,
   ...
   *  칩이 이보다 커지면 원이 아니라 **칩 상자**가 판정을 맡는다(hitTest 의 notes 갈래) —
   *  이 원은 그때도 살아 있어서, 작은 칩에 주던 관대함이 줄지 않는다. */
  hitRadiusPx: 20,
  /** 선택 링·잠김 덮개의 **최소** 반지름. 칩이 이 원을 넘어서면 링도 함께 커진다
   *  (`noteChip.ts` 의 `noteRingRadiusPx` 하나가 정한다). `physics/hitTest.ts` 의
   *  `HIT_R_MAX_PX.note`(=22, §6.5 상한) 와 같은 값이고 빈 칩 외접원(20)보다 크다. */
  ringRadiusPx: 22,
} as const;
 ...
 *  (2026-08-29 실측 반영: 0.25 × 32.5 = 8.125. 옛값 9.375 는 L=1.5 m 시절이다.) */
export const SPIN_RADIUS_MIN_PX = 8.125;
...
export const INTERACT = {
   ...
   *  똑같이 차체의 1/2 이 됐다(1/3 일 때 12.5 px, 네 토막이던 시절 7.5 px).
   *  2026-08-29 실측(L=1.3 m)으로 그 1/2 이 18.75 → **16.25 px** 이 됐다: 24 / 16.25 = 1.48.
   *  차체가 짧아지면 밴드도 좁아지므로 **더 크게 확대해야** 손가락으로 직접 잡을 수 있다.
   *  유도식을 그대로 두고 값만 남겨 두면 주석이 거짓말이 되므로 함께 옮긴다. */
  zoneDirectMinPxPerUnit: 1.48,
