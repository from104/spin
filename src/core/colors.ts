...
/** 골대 **받침판** 채움. **2026-08-31 기현 지시로 밝은 주황 → 짙은 벽돌**(*"받침대 색이 너무
 *  이질적이야"*). 옛 값 `'#fed7aa'`(흰색에 가까운 주황, 2026-08-30 하루짜리).
 *
 *  **계열은 그대로, 방향만 뒤집었다.** 옛 값도 *"기둥 테두리(`#c2410c`)와 같은 계열"* 을
 *  노린 것이었는데 그 계열의 **아주 밝은 쪽**이었다. 문제는 밝기였다 — 코트 위 어떤 것도
 *  그런 밝은 파스텔이 아니라(콘·팀색·공은 채도 높은 원색, 라인은 흰색) 판만 다른 세계에서
 *  온 물건처럼 보였다. 지금은 같은 계열의 **어두운 쪽**이라 "기둥과 한 장비" 라는 신호는
 *  유지하면서 코트 풍경에 앉는다.
 *
 *  ⚠️ **코트 대비 2.31:1 로 비텍스트 기준(3:1) 아래다. 알고 고른 값이다.** 판에는 테두리가
 *  없어(채움 사각 하나) 코트와의 명도 차 하나로만 서는데, 코트가 중간 밝기 초록이라 3:1 을
 *  넘기려면 거의 검정까지 내려가야 한다(실측: `#1f2937` 2.75 · `#10233a` 2.97 · `#0a2417`
 *  3.08 · `#1a2024` 3.08). 그 값들을 렌더해 나란히 놓고 기현님이 이쪽을 고르셨다.
 *
 *  기준을 면제하는 근거: **받침판은 정보를 나르지 않는다.** 골대가 어디 있는지는 기둥이
 *  나르고(흰 채움 + 주황 테두리, 코트 대비 충분), 판은 *"골대가 무엇 위에 서 있는가"* 를
 *  보여주는 표현이다. 같은 논리로 3:1 대상이 아닌 선례가 바로 아래 `NOTE_FOLD_FILL`(접힌
 *  모서리)이다. ⚠️ 이 면제는 **판에만** 적용된다 — 기둥 쪽 대비를 낮추는 변경은 안 된다.
 *
 *  ✅ **옛 값의 위태로움은 이 변경으로 풀렸다.** 판이 밝던 시절에는 기둥 채움(`#f5f5f5`)이
 *  판과 거의 같은 밝기여서, 기둥을 갈라 주는 것이 `#c2410c` 1.6 px 테두리 **하나뿐**이었다
 *  (*"판을 더 밝히거나 기둥 테두리를 얇게 하면 그 순간 기둥이 판에 녹는다"* 고 적어 두었다).
 *  이제 기둥 흰색이 판 위에서 **11.31:1** 이라 테두리가 없어도 읽힌다.
 *
 *  ⚠️ 훈련 콘의 주황(`CONE_COLORS[0]` = `#ff6b1a`)과는 명도가 크게 갈려 "저 주황 = 콘" 이라는
 *  눈의 규칙을 깨지 않는다 — 받침판은 코트의 붙박이 장비이고 콘은 코치가 놓는 개체다.
 *
