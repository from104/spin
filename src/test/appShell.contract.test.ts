...
describe('개체 트레이는 판의 일부다 (§6.10 재편)', () => {
  ...
  it('트레이에서 화면 밖으로 펴지는 팝오버가 없다', () => {
    // 트레이가 오른쪽 가장자리에 붙어 있어, 오른쪽으로 펴지는 것은 통째로 잘린다.
    // 콘 색 팝오버를 색깔별 상자로 대체하면서 마지막 플라이아웃이 사라졌다 —
    // 다시 들어오면 반드시 코트 쪽(왼쪽/위쪽)으로 펴야 한다.
    expect(rail, '왼쪽 레일 시절의 오른쪽 펼침이 남아 있다').not.toContain("left: '100%'");
    expect(rail, '아래로 펴면 세로 트레이에서 판 밖으로 나간다').not.toContain("top: '100%'");
  });
});
